...
- [テキスト内の数字・英字を増分しながら複製](readme-ja/SmartIncrementText.md)（v2.0.2）：項目名を［アキ］［開始値］［1つのテキストに結合］に変更、ゼロ埋めで負の値が崩れる問題を修正、増分対象が1つのときは行を表示しないよう変更、開始値が不正なときは［OK］を無効化、未使用コードの削除とラベル・命名・関数分割の整理

## 20260903

### 調整

- [ドキュメントの切替](readme-ja/SmartSwitchDocs.md)（v0.5.3）：［プレビュー］チェックボックスを追加（オフのときは［OK］をクリックしてから切り替え）、ダイアログタイトルにバージョンを表記、ドキュメントが2つのときにアクティブでない方へ確実に切り替わるよう修正、内部の命名と関数分割を整理
...
- [字形の境界を切り替えながら垂直方向に整列](readme-ja/SmartVerticalAlign.md)（v1.1.1）：［字形の境界に整列］をOFFに戻したときもプレビューに反映されるよう修正、整列位置・ショートカット・環境設定キーをテーブル化し、命名と関数分割を整理

## 20260817

### 調整

- [アートボード単位でオブジェクトをレイヤーに整理](readme-ja/ArtboardLayerOrganizer.md)（v1.3.1）：ロック／非表示の除外をOFFにしたときの移動不具合を修正、レイヤー名プレビューを追加
