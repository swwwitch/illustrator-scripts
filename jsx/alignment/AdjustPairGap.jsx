...
- 自動（既定）：整列・キーに連動します。エリア内文字＝均等配置（最終行左）、ポイント文字＝縦並びは水平整列に連動（左/中央/右）、横並びはキー側に連動（キー左→左／キー右→右）
...
- 同一セッション中：モード・キー・整列・間隔・位置・プレビュー境界をすべて記憶（#targetengine）
- Illustrator 再起動後：キー・整列・位置・プレビュー境界を復元（モード・間隔は選択内容に依存するため毎回決め直し）
