...
function showPalette() {
    ...
    /* 完全2カラム：左列＝間隔値＋左右の整列、右列＝キーオブジェクト＋テキストの行揃え /
       Full two columns: left = gap + horizontal align, right = key object + text alignment */
    var columnsRow = win.add("group");
    setupGroup(columnsRow, "row", COLUMN_SPACING);
    columnsRow.alignChildren = ["fill", "top"];

    var leftColumn = columnsRow.add("group");
    setupGroup(leftColumn, "column");
    controls.gap = buildGapPanel(leftColumn, rulerUnit, updatePreview);
    controls.align = buildAlignPanel(leftColumn, updatePreview);

    var rightColumn = columnsRow.add("group");
    setupGroup(rightColumn, "column");
    controls.anchor = buildAnchorPanel(rightColumn, updatePreview);
    controls.justify = buildJustifyPanel(rightColumn, updatePreview);
