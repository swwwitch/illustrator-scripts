...
// プレビュー時に履歴（Undo）を汚さないための管理クラス
// --------------------------------------
function PreviewManager(redrawFunc) {
    this.undoDepth = 0;
    this._redraw = (typeof redrawFunc === 'function') ? redrawFunc : function() {
        try { app.redraw(); } catch (_) {}
    };

    // Run an action as a single preview step
    this.addStep = function(func) {
        try {
            ...
            try { alert('Preview Error: ' + e); } catch (_) {}
        }
    };

    // Rollback all preview steps via app.undo()
    // Fallback to executeMenuCommand('undo') if app.undo() is unreliable in the current context.
    ...
    // Confirm: by default, keep current state but clear depth
    // (Caller can rollback before final commit and re-run once outside.)
...
function showMarginDialog(defaultValue, unit) {
    ...
    // Preview undo manager (keeps Undo history clean during live preview)
    var previewMgr = new PreviewManager(throttledRedraw);
    ...
    */
    function updatePreview(valueV) {
        // Always rollback the previous preview first to avoid piling up undo steps
        previewMgr.rollback();
        ...
        // Apply as ONE step so one rollback restores the whole preview
    ...
    okBtn.onClick = function() {
        ...
        if (parsed && parsed.valid) {
            ...
            // Important: rollback preview so Undo history is not polluted,
            // then apply final changes once outside the dialog (in main).
    ...
    cancelBtn.onClick = function() {
        // Rollback preview changes via Undo to keep history clean
        try {
            previewMgr.rollback();
        } catch (e) {
            // Fallback: restore all artboard rects directly
            for (var i = 0; i < app.activeDocument.artboards.length; i++) {
                app.activeDocument.artboards[i].artboardRect = originalRects[i].slice();
            }
            throttledRedraw();
