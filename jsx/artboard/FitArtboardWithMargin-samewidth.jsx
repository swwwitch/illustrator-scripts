...
- 選択したオブジェクトのバウンディングボックスに **上下マージン** を加味して、作業中のアートボードの **高さのみ** を自動調整します（左右＝幅は固定）。
- **選択がない場合**は、各アートボード内のオブジェクトを対象にして、**すべてのアートボード**の高さを個別に調整します。
- プレビュー境界（塗り・線を含む／含まない）を切り替え可能。
- ダイアログ位置と不透明度を記憶。

### 主な機能：

- ライブプレビュー：上下マージンの変更を即時反映（アートボードの左右は保持）。
- ターゲット切替：  
...
1) ダイアログ表示（上下マージンのみ）  
2) プレビュー：  
   - 選択あり → 選択範囲の上下境界＋マージンで **アクティブAB** の top/bottom を更新  
   - 選択なし → 各アートボード内のオブジェクトから境界を求め、**全AB** の top/bottom を更新  
3) OK：同処理を確定／Cancel：プレビューで変更したアートボードを元に戻す

### 更新履歴：

- v1.0 (2025-08-25) : 初期バージョン
...
- Adjust **height only** of the active artboard to the selection’s bounds plus **vertical margin** (width/left/right are preserved).  
- **When nothing is selected,** adjust the **height of all artboards** individually based on items inside each artboard.  
- Toggle preview bounds (visible vs geometric).  
- Remembers dialog position & opacity.
