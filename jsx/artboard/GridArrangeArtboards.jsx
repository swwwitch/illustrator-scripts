...
0行・0列、および0番は無効として扱われる。
数字だけの接頭辞は、行列指定との混同を避けるため接頭辞指定として扱わない。

非一致名や重複名は、指定した例外処理モードに従って末尾側へ自動配置する。
...
(function () {
    ...
    var LABELS = {
        dialogTitle: {
            ja: "アートボードを行列で再配置",
            en: "Rearrange Artboards in Rows and Columns"
        },
        spacingPanel: {
            ja: "間隔",
            en: "Spacing"
        ...
        exceptionRowEnd: {
            ja: "各行の末尾に",
            en: "Append to row end"
        },
        exceptionLastRow: {
            ja: "最終行に並べる",
            en: "Place in last row"
        },
        ok: {
            ja: "OK",
            en: "OK"
    ...
    /* アートボード名「行-列」（区切り文字 -, _, x）を解析してグリッド状に再配置する
       Parse '<row><sep><column>' artboard names (sep: -, _, x) and rearrange them as a grid.
