...
var SCRIPT_UPDATED = '2025-12-27';

/*
 * 概要
 * - 選択がブレンド（BlendItem/PluginItem）の場合：ブレンドは作成せず、設定／調整のみを行います。
 * - 選択がブレンド内のパス等（PathItem など）の場合でも、可能な限りブレンド本体を見つけて同様に扱います（※環境差あり）。
 * - 選択がブレンドでない場合：ブレンドを作成してから、設定／調整を行います。
 * - ステップ数／方向はダイアログ中にライブプレビュー（Undo で戻せる方式）します。
 * - ステップ数は整数のみ（小数不可）、0〜1000 の範囲で扱います（0 可）。
 * - 既にブレンドを選択している場合、そのステップ数（blendOptions.steps）を取得してデフォルト表示します（取得できない場合は 10）。
 * - ステップ数の下にスライダーを表示：
 *    - 通常：0〜32
 *    - option：0〜128
...
// 選択しているオブジェクトが（PluginItem/BlendItem）でない場合のみ、ブレンドを作成
// （既に PluginItem/BlendItem の場合は、作成せずオプション/調整のみ行う）
    if (!isAllPluginItems(selection)) {
        try {
        ...
        // Already PluginItem; keep current selection
        selection = doc.selection;
        if (!selection || selection.length <= 0) {
            return;
        }
...
// 選択オブジェクトが「GroupItem のみ」かどうかを判定する
// 「ブレンド軸を置き換え」オプションの有効／無効制御に使用
function isOnlyGroupItems(selection) {
    try {
        if (!selection || selection.length <= 0) {
            return false;
        }
        for (var i = 0; i < selection.length; i++) {
            if (!selection[i] || selection[i].typename !== 'GroupItem') {
                return false;
            }
        }
        return true;
...
function getInput(selection) {
    ...
    // Default step: use selected blend's steps if available, otherwise 10
    var _defaultStep = 10;
    try {
    ...
    // 選択が GroupItem のみの場合は
    // 「ブレンド軸を置き換え」をディム表示にする
    function updateAdjustAvailability() {
        try {
            var onlyGroup = isOnlyGroupItems(selection);
            rbAdjustReplace.enabled = !onlyGroup;
