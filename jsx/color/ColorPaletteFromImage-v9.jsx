...
/* 選択を正規化 / Normalize selection
   - selection は常に配列として扱う
   - TextRange はそのまま保持
   - TextFrame は textRange に正規化して文字単位の色抽出に統一
   - GroupItem はそのまま保持（内部は後段で再帰処理） */
...
function normalizeSelection(selection) {
    ...
    // 通常の配列
    if (!selection.length) return normalized;
    ...
    for (var i = 0; i < selection.length; i++) {
        ...
        try {
            if (item.typename === "TextRange") {
                ...
                normalized.push(item.textRange);
            } else {
                normalized.push(item);
            }
        } catch (e) {
            __logError(e, "normalizeSelection");
        }
    }
...
/* 正規化済み選択から処理対象を構築 / Build process targets from normalized selection */
function buildProcessTargets(normalizedSelection) {
    var rasterItems = [];
    var vectorItems = [];

    for (var i = 0; i < normalizedSelection.length; i++) {
        var item = normalizedSelection[i];
        if (!item) continue;
...
/* TextRange の配置基準を得る / Resolve palette anchor for TextRange */
function getTextRangeAnchor(item) {
    if (!item) return item;
    if (item.typename !== "TextRange") return item;

    try {
        var p = item;
        while (p && p.typename !== "TextFrame") {
            p = p.parent;
        }
        if (p && p.typename === "TextFrame") return p;
    } catch (e) {
        __logError(e, "getTextRangeAnchor");
    }
    return item;
}
...
if (app.documents.length === 0) {
    ...
    var normalizedSelection = normalizeSelection(rawSelection);
    var targets = buildProcessTargets(normalizedSelection);
    var rasterItems = targets.rasterItems;
    var vectorItems = targets.vectorItems;
    ...
    if (vectorItems.length > 0) {
        var vectorAnchor = getTextRangeAnchor(vectorItems[0]);
        jobs.push({ type: "vector", originalItem: vectorAnchor }); /* パレット配置の基準 / Anchor for palette position */
    }
                ...
                try {
                    ...
                    for (var i = 0; i < jobs.length; i++) {
                        if (jobs[i].type === "vector") {
                            ...
                            for (var v = vectorItems.length - 1; v >= 0; v--) {
                                var srcVec = vectorItems[v];
                                if (srcVec.typename === "TextRange") {
                                    // TextRange は duplicate が不安定なので、ここでは複製せず色抽出を本体で行う。
                                    // 作業グループには何も追加しない。
                                } else {
                                    var vdup = srcVec.duplicate();
                    ...
                    for (var j = 0; j < itemsToProcess.length; j++) {
                        try {
                            ...
                            var colors;
                            if (job.type === "vector") {
                                // ベクター: 正規化済み選択から直接カラーを抽出（ラスタライズ不要）
                                colors = [];
                                for (var nv = 0; nv < vectorItems.length; nv++) {
                                    collectFillColors(vectorItems[nv], colors);
                                }
...
function collectFillColors(item, out) {
    ...
    if (item.typename === "PathItem") {
        if (item.filled && item.fillColor) {
            ...
            if (c.typename !== "NoColor" && c.typename !== "PatternColor" && c.typename !== "GradientColor") {
                var pushColor = c;
                if (c.typename === "SpotColor" && c.spot && c.spot.color) {
                    pushColor = c.spot.color;
                }
                var a = 1;
                try { a = Math.abs(item.area); } catch (e) { __logError(e); }
                if (a < 1) a = 1;
                out.push({ color: pushColor, area: a });
            }
        }
        return out;
    }
    if (item.typename === "TextFrame" || item.typename === "TextRange") {
        try {
            var chars = item.textRange ? item.textRange.characters : item.characters;
            for (var ci = 0; ci < chars.length; ci++) {
                try {
                    var ch = chars[ci];
                    var charColor = ch.characterAttributes.fillColor;
                    if (charColor && charColor.typename !== "NoColor" && charColor.typename !== "PatternColor" && charColor.typename !== "GradientColor") {
                        var pushCharColor = charColor;
                        if (charColor.typename === "SpotColor" && charColor.spot && charColor.spot.color) {
                            pushCharColor = charColor.spot.color;
                        }
                        out.push({ color: pushCharColor, area: 1 });
                    }
                } catch (eChar) { __logError(eChar, "collectFillColors char"); }
            }
        } catch (eText) { __logError(eText, "collectFillColors text"); }
        return out;
    }
...
function drawSwatchSquares(doc, originalItem, swatchSource, outOpt, containerGroup) {
    ...
    /* 元オブジェクトの位置・サイズを取得 / Get original object position and size
       TextFrame では width / height が 0 や不安定なことがあるため、geometricBounds を優先する。 */
    var gb = null;
    try {
        gb = originalItem.geometricBounds; // [left, top, right, bottom]
    } catch (eGb) {
        __logError(eGb, "drawSwatchSquares geometricBounds");
    }

    var imgLeft, imgTop, imgWidth, imgBottom;
    if (gb && gb.length >= 4) {
        imgLeft = gb[0];
        imgTop = gb[1];
        imgWidth = gb[2] - gb[0];
        ...
        imgLeft = originalItem.left;
        imgTop = originalItem.top;
        imgWidth = originalItem.width;
        imgBottom = imgTop - originalItem.height;
    }
...
function colorToCMYKVals(color) {
    ...
    if (color && color.typename === "SpotColor" && color.spot && color.spot.color) {
        return colorToCMYKVals(color.spot.color);
    }
    try {
...
function colorToRGB(color) {
    ...
    if (color.typename === "SpotColor" && color.spot && color.spot.color) {
        return colorToRGB(color.spot.color);
    }
