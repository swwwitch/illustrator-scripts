...
// Duplicate current selection and run Blend on the duplicates (keep original selection intact)
function duplicateSelectionAndBlend(doc) {
    try {
        if (!doc || !doc.selection || doc.selection.length < 2) return null;

        // Snapshot original selection
        var original = [];
        for (var i = 0; i < doc.selection.length; i++) {
            try { original.push(doc.selection[i]); } catch (eO) { }
        }
        ...
        if (dups.length < 2) {
            ...
            try {
                doc.selection = null;
                doc.selection = original;
            } catch (eR1) { }
            return null;
        }

        // Select duplicates and run Blend
        try { doc.selection = null; } catch (eC) { }
        for (var k = 0; k < dups.length; k++) {
            try { dups[k].selected = true; } catch (eS) { }
        }

        // Try known Blend menu commands (varies by locale/version)
        var blended = null;
        try {
        ...
        // Capture result selection (often the new BlendItem)
        try {
            if (doc.selection && doc.selection.length) {
                blended = doc.selection[0];
            }
        } catch (eSel) { }
...
function main() {
    ...
    // Duplicate selected objects and run Blend on the duplicates (silent)
    // 複製したオブジェクトに対してブレンドを実行（元の選択は維持）
    try { duplicateSelectionAndBlend(doc); } catch (eBlend) { }
