...
/*
  ...
  ・オプションにより、グローバルカラー化／グラデーション作成／長方形作成を切り替え可能
  ・グラデーションを作成する場合、各ストップには登録したカラー（グローバルON時はグローバル）を設定
  ・長方形を作成する場合、生成したグラデーションを適用
...
function main() {
    ...
    try {
        ...
        var cbGlobal = dlg.add('checkbox', undefined, L('globalColor'));
        cbGlobal.value = true;

        var cbGradient = dlg.add('checkbox', undefined, L('createGradient'));
        cbGradient.value = true;

        var cbRect = dlg.add('checkbox', undefined, L('createRect'));
        cbRect.value = true;
