...
配色はスウォッチの選択・選択オブジェクトの塗り色・スウォッチグループから取り込め、適用単位（オブジェクト／1文字／単語／行／段落）と適用順（そのまま／逆順／ランダム／完全ランダム）を変えるたびにライブプレビューします。
...
Colors are captured from the selected swatches, from the fills of the selected objects or from a swatch group, and every change to the application unit (object, character, word, line or paragraph) or order (as-is, reversed, random or fully random) is previewed live.
...
(function () {
    ...
    var LABELS = {
        ...
        source: {
            ...
            choose:  { ja: "（グループを選択）", en: "(choose a group)" },
        ...
        tooltip: {
            ...
            colors:        { ja: "適用に使う配色。下のボタンやスウォッチグループから取り込み直せます。", en: "Colors used for applying; re-capture with the buttons below or from a swatch group." },
            group:         { ja: "ドキュメントのスウォッチグループから配色を取り込みます。", en: "Captures colors from a swatch group in the document." },
    ...
    function showPalette() {
        ...
        /* スウォッチグループから取り込む（未分類グループは一覧に出さない）。ポップアップは長い名前でも
           パレットを広げないよう、最小幅を小さくして残り幅いっぱいに伸ばす
           Capture from a swatch group (the uncategorized group is not listed). The dropdown keeps a small
           preferred width and fills the rest, so a long name cannot widen the palette */
        var groupRow = colorPanel.add("group");
        setupRow(groupRow, "fill");
        groupRow.alignChildren = ["left", "center"];
        groupRow.margins = [0, 5, 0, 0]; /* ボタンとの間に余白 / margin above the row */
        groupRow.add("statictext", undefined, L("source.group"));
        var groupDropdown = groupRow.add("dropdownlist", undefined, []);
        groupDropdown.helpTip = L("tooltip.group");
        groupDropdown.preferredSize.width = 100;
        groupDropdown.alignment = ["fill", "center"];
        var swatchGroups = [];

        /* selection への代入でも onChange が走るため、プログラム的な選択変更中は無視する
           Assigning selection fires onChange too; ignore it while changing the selection programmatically */
        var suspendGroupChange = false;
        function setGroupSelection(index) {
            suspendGroupChange = true;
            try { groupDropdown.selection = index; } finally { suspendGroupChange = false; }
        }

        /* グループ一覧を読み直してポップアップを作り直す（先頭はプレースホルダー）
           Reload the group list and rebuild the dropdown (index 0 is a placeholder) */
        function rebuildGroupDropdown(groups) {
            swatchGroups = groups || readSwatchGroupNames();
            groupDropdown.removeAll();
            groupDropdown.add("item", swatchGroups.length > 0 ? L("source.choose") : L("source.noGroup"));
            for (var i = 0; i < swatchGroups.length; i++) { groupDropdown.add("item", swatchGroups[i].name); }
            setGroupSelection(0);
            groupDropdown.enabled = (swatchGroups.length > 0);
            STATE.lastGroupSig = swatchGroupsSignature(swatchGroups);
        }
        ...
        function onFromSwatches() {
            ...
            loadedColorValues = [];
            setGroupSelection(0); /* 取り込み元が変わったのでグループ表示を戻す / colors no longer come from a group */
        ...
        /* スウォッチグループを選んだら、その配色を取り込んで適用（プレースホルダーは無視）。
           中身が空（[なし]のみ）のグループは配色を変えずに表示だけ戻す
           Selecting a group captures its colors and applies them (the placeholder is ignored);
           an empty group (only the None swatch) leaves the colors untouched */
        groupDropdown.onChange = function () {
            if (suspendGroupChange) return;
            if (!groupDropdown.selection || groupDropdown.selection.index === 0) return;
            var fresh = readSwatchGroupInfo(swatchGroups[groupDropdown.selection.index - 1].index);
            if (!fresh.ok || fresh.swatchNames.length === 0) { setGroupSelection(0); return; }
            loadedSwatchNames = fresh.swatchNames;
            loadedColorValues = [];
            refreshChips(fresh.chips);
            runPreview();
        };
        ...
        win.onActivate = function () {
            ...
            /* 書類の切り替えやグループの増減に追従（構成が変わったときだけ作り直す）
               Follow document switches and added/removed groups (rebuild only when the list changed) */
            var freshGroups = readSwatchGroupNames();
            if (swatchGroupsSignature(freshGroups) !== STATE.lastGroupSig) { rebuildGroupDropdown(freshGroups); }
        };
    ...
    var WORKER_FUNCS = [
        ...
        workerReadSelection, workerReadObjectColors, workerReadStats, workerReadSwatchGroupNames, workerReadSwatchGroup,
        selectionTextStatsW, countParagraphsW,
    ...
    /* スウォッチグループの一覧（未分類は除外）を委譲取得。要素は { index, name }
       index は doc.swatchGroups の実インデックス（未分類を飛ばすため一覧の並びとずれる）
       Delegate reading the swatch group list; index is the real doc.swatchGroups index */
    function readSwatchGroupNames() {
        var groups = [];
        var raw = runWorker("workerReadSwatchGroupNames()");
        if (!raw || raw.indexOf("OK|") !== 0) return groups;
        var parts = raw.split("|");
        for (var i = 1; i + 1 < parts.length; i += 2) {
            groups.push({ index: parseInt(parts[i], 10), name: decodeURIComponent(parts[i + 1]) });
        }
        return groups;
    }

    /* 指定スウォッチグループの配色を委譲取得（チップRGB＋encode済みスウォッチ名）
       Delegate reading one swatch group's colors (chip RGB + URI-encoded swatch names) */
    function readSwatchGroupInfo(groupIndex) {
        var info = { ok: false, chips: [], swatchNames: [] };
        var raw = runWorker("workerReadSwatchGroup(" + groupIndex + ")");
        if (!raw || raw.indexOf("OK|") !== 0) return info;
        info.ok = true;
        var parts = raw.split("|");
        info.chips = parseChips(parts[1]);
        if (parts[2]) { info.swatchNames = parts[2].split(","); }
        return info;
    }

    /* グループ一覧の構成を表す署名（作り直しの要否判定）/ Signature of the group list (to decide a rebuild) */
    function swatchGroupsSignature(groups) {
        var keys = [];
        for (var i = 0; i < groups.length; i++) { keys.push(groups[i].index + ":" + groups[i].name); }
        return keys.join(",");
    }
    ...
    /* スウォッチグループ名を返す "OK|<index>|<encName>|<index>|<encName>..."（未分類＝名前なしは除外）。
       名前だけ読むので軽い（getAllSwatches は呼ばない）/ Return swatch group names; cheap (no getAllSwatches) */
    function workerReadSwatchGroupNames() {
        if (app.documents.length === 0) return "NODOC";
        var doc = app.activeDocument;
        var parts = [];
        for (var i = 0; i < doc.swatchGroups.length; i++) {
            var name;
            try { name = doc.swatchGroups[i].name; } catch (e) { continue; }
            if (name === "") { continue; }
            parts.push(i);
    ...
    /* 指定スウォッチグループの配色を返す "OK|<r,g,b;...>|<encName,...>"（NoColor スウォッチは除外）
       Return one swatch group's colors (the None swatch is excluded) */
    function workerReadSwatchGroup(groupIndex) {
        if (app.documents.length === 0) return "NODOC";
        var doc = app.activeDocument;
        if (groupIndex < 0 || groupIndex >= doc.swatchGroups.length) return "ERR:range";
        var swatches;
        try { swatches = doc.swatchGroups[groupIndex].getAllSwatches(); } catch (e) { return "ERR:group"; }
        var chipParts = [];
        var nameParts = [];
        for (var i = 0; i < swatches.length; i++) {
            var color = swatches[i].color;
            if (color.typename === "NoColor") { continue; }
            chipParts.push(colorToRGB255W(color));
            nameParts.push(encodeURIComponent(swatches[i].name));
        }
        return "OK|" + chipParts.join(";") + "|" + nameParts.join(",");
    }
