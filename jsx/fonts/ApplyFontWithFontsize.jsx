...
(function () {
    ...
    // サイズ・行送りは指定があるものだけ反映。行送りは「絶対値」では入れず、
    // 行送り÷サイズの百分率を自動行送りの値（autoLeadingAmount）に代入し、行送りは自動（autoLeading=ON）にする。
    // 例）12pt↓16pt → 16/12 = 約133.3% を自動行送りに設定。適用中に例外が出たら false を返す。
    function applyFontSpec(paragraph, font, size, leading) {
        try {
            ...
            if (leading !== null && leading !== undefined
                ...
                // 行送り（絶対値）をサイズに対する百分率へ変換し、自動行送りの値に代入する
                paragraph.paragraphAttributes.autoLeadingAmount = (leading / size) * 100;
