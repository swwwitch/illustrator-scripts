...
履歴
v1.0.0  初期バージョン
v1.1.1  あいまい一致・対話ピッカーまわりの安定化
v1.2.0  行末に併記されたフォントサイズの検出・適用に対応
v1.3.0  行送りの併記に対応／単位 Q をサポート／サイズ・行送り情報がない行はスキップ
v1.3.1  区切り文字に「/」を追加（フォント名↔サイズ／サイズ↔行送り／単位なしサイズ）
v1.3.2  クリップボードコピー後に元の選択状態を復元／内部リファクタ・コメント整理
...
(function () {
    ...
    // 処理対象が確定してから、全インストールフォントを索引化する（重い処理なので選択チェック後に実行）
    ...
    var unappliedFrames = [];
    var unappliedTexts = [];
    var pickMemo = {}; // fontName -> TextFont（適用）/ null（スキップ）
    ...
    for (var i = 0; i < pendingPicks.length; i++) {
        ...
        // 同じフォント名を既に決めていれば、その結果を再利用（ピッカーを出さない）。
        // サイズ・行送りは行ごとに異なりうるので pick の値を都度適用する。
        if (pickMemo.hasOwnProperty(pick.fontName)) {
            var remembered = pickMemo[pick.fontName];
            var reapplied = remembered
                ? applyFontSpecToLine(pick.frame, pick.index, remembered, pick.fontSize, pick.fontLeading)
                : false;
        ...
        var picked = showFontPicker(picker, pick.frame, pick.index, pick.lineText, pick.initialFont, pick.fontSize, pick.fontLeading);
        ...
        pickMemo[pick.fontName] = picked; // TextFont or null
        if (picked === null) {
    ...
    function dlog(message) {
        if (!DEBUG_LOG) return;
        try {
            var logFile = new File(Folder.desktop + "/ApplyFontWithFontsize-debug.log");
    ...
    function showResultDialog(lines) {
        ...
        var dialog = new Window("dialog", L(LABELS.result.title) + " " + SCRIPT_VERSION);
    ...
    function copyTextToClipboard(textToCopy) {
        var tempFrame = null;
        var previousSelection = doc.selection; // コピー後に元へ戻すため控える
        try {
            ...
            // 一時フレーム用に潰した選択を、呼び出し前の状態へ復元する
            try { doc.selection = previousSelection; } catch (e3) { doc.selection = null; }
        }
    }
    ...
    function showFontPicker(picker, targetFrame, paragraphIndex, lineText, initialFont, fontSize, fontLeading) {
        ...
        return handleFontPickerResult(result, targetFrame, paragraphIndex, picker.familyList, picker.styleList, fontSize, fontLeading);
    }
    ...
    function handleFontPickerResult(result, targetFrame, paragraphIndex, familyList, styleList, fontSize, fontLeading) {
        if (result === 1 && familyList.selection && styleList.selection) {
            ...
            if (chosenFont && applyFontSpecToLine(targetFrame, paragraphIndex, chosenFont, fontSize, fontLeading)) {
                return chosenFont;
            }
        }
    ...
    // 一致が無くなった場合は空のままにする（選択なしは呼び出し側でガードしている）。
    ...
    // 1つのテキストフレームの各行（段落）について、サイズ併記のある行だけを対象に、
    // 厳密一致のフォントは自動適用し、あいまい／未一致の行は
    // { frame, index, lineText, fontName, fontSize, fontLeading, initialFont } を pendingPicks に積む。
    // サイズ併記の無い行（フォント名だけの行）はスキップする。
    ...
    function autoApplyAndQueue(textFrame, pendingPicks) {
        ...
        for (var j = paragraphCount - 1; j >= 0; j--) {
            ...
            // 行末に併記されたフォントサイズ・行送り（例「… 12pt↓16pt」）を切り出す。
            // 検索は名前部分（parsed.name）だけで行い、サイズ・行送りは適用時に反映する。
            var parsed = parseFontSpec(lineText);

            // 文字サイズ・行送りの情報がない行は処理対象外（スキップ）。
            // フォント名だけの行はフォント適用もピッカー送りも行わない。
            if (parsed.size === null && parsed.leading === null) continue;

            // フォントを検索（{ font, confident }）
            var match = findFont(parsed.name);

            // 厳密一致 → 自動適用（サイズ・行送りの併記があればそれも）
            var autoApplied = false;
            if (match.confident && match.font) {
                autoApplied = applyFontSpec(paragraph, match.font, parsed.size, parsed.leading);
            }
    ...
    // オブジェクトのプロパティへ値を代入する。代入できない（未対応の）プロパティは無視する
    ...
    function unlockContainer(item) {
        setPropertySafely(item, "locked", false);
        setPropertySafely(item, "visible", true);
        setPropertySafely(item, "hidden", false);
    }
    ...
    // 例: "ヒラギノ角ゴシック W3 12pt"        → { name: "ヒラギノ角ゴシック W3", size: 12,   leading: null }
    //     "ヒラギノ角ゴシック W3 12pt↓16pt"   → { name: "ヒラギノ角ゴシック W3", size: 12,   leading: 16 }
    //     "Helvetica Neue, 14"              → { name: "Helvetica Neue",       size: 14,   leading: null }
    //     "DIN 2014 12pt"                   → { name: "DIN 2014",             size: 12,   leading: null }
    //     "DIN 2014"（サイズ併記なし）       → { name: "DIN 2014",             size: null, leading: null }
    //
    // ・フォント名とサイズの区切り：半角/全角スペース・タブ・「、」「,」「/」
    // ・サイズと行送りの区切り    ：上記に加えて「↓」
    // ・サイズ／行送りは数値（小数可）＋単位（pt/px/Q）。Q（級）は 1Q = 0.25mm として
    //   ポイントへ換算（px/pt/単位なしは 1px = 1pt のまま）。返す size / leading は常にポイント値。
    //
    // 肝は「サイズは “単位付き” か “「、」「,」「/」区切り” のどちらかで必ずアンカーされる」こと。
    // フォント名末尾の数字（例 "DIN 2014" / "Univers 45"）は単位なし・スペース区切りなので、
    // サイズと誤認されない（その結果、サイズ併記が無い行として後段でスキップされる）。
    // 行送りはサイズが確定した直後の数値だけを見る（単位・区切りは任意）。
    function parseFontSpec(rawText) {
        var text = String(rawText).replace(/^[\s　]+|[\s　]+$/g, "");
        var result = { name: text, size: null, leading: null };

        // 末尾の行送りは任意：[区切り(↓含む) → 数値 →（任意で単位）]
        var trailingLeading = "(?:[ \\t　、,/↓]+([0-9]+(?:\\.[0-9]+)?)(pt|px|q)?)?$";

        // パターン1：サイズに単位（pt/px/Q）が付くケース。
        //   フォント名↔サイズの区切り＝半角/全角スペース・タブ・「、」「,」「/」。
        //   単位が必須なので、名前末尾の裸の数字（"DIN 2014" 等）はサイズとして拾われない。
        var sizeWithUnit = new RegExp(
            "^([\\s\\S]*?)[ \\t　、,/]+([0-9]+(?:\\.[0-9]+)?)(pt|px|q)" + trailingLeading, "i");

        // パターン2：単位は無いが「、」「,」「/」で明示的に区切られているケース（例 "Helvetica Neue, 14" / "Helvetica Neue/14"）。
        var sizeWithListSeparator = new RegExp(
            "^([\\s\\S]*?)[ \\t　]*[、,/][ \\t　]*([0-9]+(?:\\.[0-9]+)?)(pt|px|q)?" + trailingLeading, "i");

        // どちらも groups: 1=名前 / 2=サイズ数値 / 3=サイズ単位 / 4=行送り数値 / 5=行送り単位
        var match = text.match(sizeWithUnit) || text.match(sizeWithListSeparator);
        if (!match) return result;

        var name = String(match[1]).replace(/[\s　]+$/g, "");
        if (name === "") return result; // 名前が空ならサイズ扱いしない

        var size = unitToPoints(match[2], match[3]);
        if (size === null) return result;

        result.name = name;
        result.size = size;

        // サイズが確定したときだけ行送りを見る（直後に併記された数値はほぼ行送りと断定できる）
        if (match[4]) {
            result.leading = unitToPoints(match[4], match[5]);
        }

        return result;
    }

    // 数値文字列＋単位（pt/px/Q/単位なし）をポイント値へ換算する。正の値でなければ null。
    // Q（級）は 1Q = 0.25mm。px/pt/単位なしは 1px = 1pt としてそのまま扱う。
    function unitToPoints(numberText, unit) {
        var value = parseFloat(numberText);
        if (!(value > 0)) return null;
        if (unit && String(unit).toLowerCase() === "q") {
            return value * (72 / 25.4 / 4); // 1Q = 0.25mm → pt
        }
        return value;
    }

    // 段落へフォント・サイズ・行送り（サイズ・行送りはポイント値）を適用する。成否を返す。
    // サイズ・行送りは指定があるものだけ反映。行送りは autoLeading を OFF にしてから値を入れる
    // （自動行送りに上書きされないため）。適用中に例外が出たら false を返す。
    function applyFontSpec(paragraph, font, size, leading) {
        try {
            var attrs = paragraph.characterAttributes;
            if (font) attrs.textFont = font;
            if (size !== null && size !== undefined) attrs.size = size;
    ...
    // frame＋段落インデックスから段落をライブ取得し、フォント等を適用する。
    // 取得失敗・適用失敗はいずれも false（保持参照の無効化クラッシュを避けるため都度取り直す）。
    function applyFontSpecToLine(frame, index, font, size, leading) {
        var paragraph = getLiveParagraph(frame, index);
        if (!paragraph) return false;
        return applyFontSpec(paragraph, font, size, leading);
    }
