...
(function () {
    ...
    function classifyFontChange(oldName) {
        ...
        var nameHead = buildConvertedNameHead(parsed); // ウエイトを除いた変換後の先頭
        if (nameHead === null) return;                 // 変換対象外（CID 非変換など）

        var desiredName = nameHead + parsed.weight;
        if (desiredName === oldName) return; // 変化なし
        ...
        var nearWeight = findNearestWeight(nameHead, parsed.weight);
        if (nearWeight) {
            weightSubChanges.push({
                oldName: oldName,
                newName: nameHead + nearWeight,
                oldWeight: parsed.weight,
