...
(function () {
    ...
    function buildMarginUI(parent) {
        ...
        marginPanel = parent.add(
            "panel",
            undefined,
            L("panelMargin") + "（" + getCurrentRulerUnitLabel() + "）"
        );
        ...
        editArtboardMarginTop.characters = 3;
        changeValueByArrowKey(editArtboardMarginTop, false);
        // [Unit label removed from Top row]
        ...
        editArtboardMarginLeft.characters = 3;
        ...
        editArtboardMarginRight.characters = 3;
        ...
        editArtboardMarginBottom.characters = 3;
        changeValueByArrowKey(editArtboardMarginBottom, false);
        // [Unit label removed from Bottom row]
    ...
    // アートボード基準（=オブジェクト未選択）のときだけ有効
    framePanel.enabled = !!_usingArtboardBase;
    ...
    function applyFrameEnabledState() {
        // Frameは「オブジェクト未選択（アートボード基準）」のときのみ利用可能
        if (!_usingArtboardBase) {
            try { framePanel.enabled = false; } catch (_) { }
            try { chkFrameEnable.value = false; } catch (_) { }
            try { editFrameWidth.text = "0"; } catch (_) { }
            try { editFrameWidth.enabled = false; } catch (_) { }
            try { if (chkBleed) { chkBleed.value = false; chkBleed.enabled = false; chkBleed.visible = false; } } catch (_) { }
            try { if (chkFrameRound) { chkFrameRound.value = false; chkFrameRound.enabled = false; } } catch (_) { }
            try { if (editFrameRound) { editFrameRound.text = "0"; editFrameRound.enabled = false; } } catch (_) { }
            return;
        } else {
            try { framePanel.enabled = true; } catch (_) { }
        }
        try {
            var enabled = !!chkFrameEnable.value;
    ...
    if (result == 1) {
        ...
        // オブジェクト選択時は元オブジェクトをガイド化して残す
        convertSelectedTargetsToGuides();
        ...
        if (!isNaN(_okLen) && _okLen !== 0) {
            ...
            for (var i = 0; i < targetItems.length; i++) {
                try {
                    ...
                    var _isGuide = false;
                    try { _isGuide = (targetItems[i].guides === true); } catch (_) { _isGuide = false; }
                    if (_isGuide) continue;
    ...
    function collectOptions() {
        ...
        // 念のため：辺の伸縮OFFのときは distPt を 0 に固定
        try { if (chkEnableLen && !chkEnableLen.value) distPt = 0; } catch (_) { }
        try { if (chkKeepOuter && !chkKeepOuter.value) distPt = 0; } catch (_) { }
    ...
    // 選択オブジェクトの bounds を「基準矩形（線あり）」として作る（マージン0相当）
    // ※元オブジェクトの stroke/noColor/hidden に依存しないため、外枠線が消えない
    function createSelectionBaseRect(pathItem) {
        try {
            if (!pathItem) return null;

            var b = null;
            try { b = pathItem.geometricBounds; } catch (_) { b = null; }
            ...
            var lay = null;
            try { lay = pathItem.layer; } catch (_) { lay = null; }
            if (!lay) lay = doc.activeLayer;

            // 線だけの矩形を新規作成（1pt / K100）
            var rect = lay.pathItems.rectangle(T, L, w, h);
            try { rect.filled = false; } catch (_) { }
            try { rect.stroked = true; } catch (_) { }
            try { rect.strokeWidth = 1; } catch (_) { }
            try {
                var k = new CMYKColor();
                k.cyan = 0; k.magenta = 0; k.yellow = 0; k.black = 100;
                rect.strokeColor = k;
            } catch (_) { }
            try { rect.hidden = false; } catch (_) { }
            try { rect.guides = false; } catch (_) { }

            // 元オブジェクトの直上へ（塗りに隠れない）
            try { rect.move(pathItem, ElementPlacement.PLACEAFTER); } catch (_) {
                try { rect.zOrder(ZOrderMethod.BRINGTOFRONT); } catch (__) { }
            }

            // 外枠として識別（KeepOuter OFF で削除される対象にする）ではなく、__SelectionBaseRect__でタグ付け
            try { rect.note = "__SelectionBaseRect__"; } catch (_) { }
            try { rect.name = "__SelectionBaseRect__"; } catch (_) { }
    ...
    function generateFromOptions(opt, isFinal) {
        // baseItems: 外枠／タイトル／内側の計算に使う基準（アートボード基準 or 選択オブジェクト基準）
        // 選択時は、元オブジェクトの stroke/noColor に依存しない「線あり基準矩形」を作って使う（マージン0）
        var baseItems = targetItems;
        var _selectionBaseRects = [];
        if (!_usingArtboardBase) {
            baseItems = [];
            for (var bi = 0; bi < targetItems.length; bi++) {
                var src = targetItems[bi];
                // ガイドは基準にしない（OK後にガイド化されるため）
                var _isGuide = false;
                try { _isGuide = (src && src.guides === true); } catch (_) { _isGuide = false; }
                if (_isGuide) continue;

                var br = createSelectionBaseRect(src);
                if (br) {
                    _selectionBaseRects.push(br);
                    tempPreviewItems.push(br);
                    baseItems.push(br);
                } else {
                    baseItems.push(src);
                }
        ...
        if (opt.distPt === 0) {
            ...
            // 辺の伸縮OFFのとき：選択時は基準矩形（線あり）を外枠として使う（元オブジェクトのstrokeに依存しない）
            if (!_usingArtboardBase) {
                var _showBase = (chkKeepOuter && chkKeepOuter.value);
                for (var bi2 = 0; bi2 < _selectionBaseRects.length; bi2++) {
                    try { _selectionBaseRects[bi2].hidden = !_showBase; } catch (_) { }
                }
            }
            // タイトル帯（塗り）
            if (opt.titleSizePt > 0) {
                for (var i = 0; i < baseItems.length; i++) {
                    createTitleFill(baseItems[i], opt.titleSizePt);
                }
            }
            ...
            if (opt.titleSizePt > 0) {
                for (var i = 0; i < baseItems.length; i++) {
                    createTitleDivider(baseItems[i], opt.titleSizePt, opt.distPt);
                }
            }
            ...
            for (var i = 0; i < targetItems.length; i++) {
                try {
                    ...
                    // distPt!=0 の分解表示で hidden=true になっている可能性があるため、ここで必ず復帰させる
                    targetItems[i].hidden = !showOuterRect;
                } catch (_) { }
            }
            if (!_usingArtboardBase) {
                for (var bi3 = 0; bi3 < _selectionBaseRects.length; bi3++) {
                    try { _selectionBaseRects[bi3].hidden = !showOuterRect; } catch (_) { }
                }
            }

            // 外側エリア：角丸（辺の伸縮OFFのときのみ）
            // - プレビュー：一時オブジェクトに角丸を適用（元を汚さない）
            // - OK後：最終生成物に角丸を適用
            try {
                ...
                if (useOuterRound) {
                    var rPt = rVal * opt.factor;

                    if (_usingArtboardBase) {
                        // アートボード基準：外枠は _artboardBaseRect
                        if (isFinal) {
                            try { if (_artboardBaseRect) applyRoundCornersEffect(_artboardBaseRect, rPt); } catch (_) { }
                        } else {
                            // プレビュー時：元外枠を隠し、同boundsの矩形を作って角丸を適用
                            try {
                                if (_artboardBaseRect) {
                                    var b0 = _artboardBaseRect.geometricBounds; // [L,T,R,B]
                                    var L0 = b0[0], T0 = b0[1], R0 = b0[2], B0 = b0[3];
                                    var w0 = R0 - L0, h0 = T0 - B0;
                                    if (w0 > 0 && h0 > 0) {
                                        try { _artboardBaseRect.hidden = true; } catch (_) { }

                                        var rr0 = _artboardBaseRect.layer.pathItems.rectangle(T0, L0, w0, h0);
                                        try { rr0.filled = false; } catch (_) { }
                                        try { rr0.stroked = true; } catch (_) { }
                                        try { rr0.strokeWidth = 1; } catch (_) { }
                                        try {
                                            var k0 = new CMYKColor();
                                            k0.cyan = 0; k0.magenta = 0; k0.yellow = 0; k0.black = 100;
                                            rr0.strokeColor = k0;
                                        } catch (_) { }
                                        try { rr0.note = "__OuterRoundPreview__"; } catch (_) { }
                                        try { rr0.name = "__OuterRoundPreview__"; } catch (_) { }

                                        applyRoundCornersEffect(rr0, rPt);
                                        tempPreviewItems.push(rr0);
                                    }
                                }
                            } catch (_) { }
                        }
                    } else {
                        // 選択時：基準矩形（_selectionBaseRects）に角丸を適用
                        // 角丸適用後に隠されないよう、念のため showOuterRect を維持
                        for (var ri = 0; ri < _selectionBaseRects.length; ri++) {
                            try {
                                applyRoundCornersEffect(_selectionBaseRects[ri], rPt);
                                _selectionBaseRects[ri].hidden = false;
                            } catch (_) { }
                        }
                    }
                }
            ...
            // 内側エリア（オフセットが0でも描画）
            for (var i = 0; i < baseItems.length; i++) {
                var innerB = getInnerAreaBounds(baseItems[i], opt.titleSizePt);
                if (innerB) {
                    createInnerBox(
                        baseItems[i],
                        innerB,
            ...
            // 辺の伸縮OFF時：選択基準矩形の線が後から生成される塗り/内側要素に隠れることがあるため、最前面へ
            if (!_usingArtboardBase && chkKeepOuter && chkKeepOuter.value) {
                for (var zi = 0; zi < _selectionBaseRects.length; zi++) {
                    try {
                        _selectionBaseRects[zi].hidden = false;
                        _selectionBaseRects[zi].zOrder(ZOrderMethod.BRINGTOFRONT);
                    } catch (_) { }
                }
            }
        ...
        if (chkKeepOuter.value) {
            for (var i = 0; i < baseItems.length; i++) {
                createShortenedLines(baseItems[i], opt.distPt);
            }
        }
        ...
        // 内側エリア
        for (var i = 0; i < baseItems.length; i++) {
            var innerB2 = getInnerAreaBounds(baseItems[i], opt.titleSizePt);
            if (innerB2) {
                createInnerBox(
                    baseItems[i],
                    innerB2,
    ...
    function getEffectiveLenValue() {
        // 外枠を残さない／辺の伸縮OFF のときは 0 扱い
        try { if (chkKeepOuter && !chkKeepOuter.value) return 0; } catch (_) { }
        try { if (chkEnableLen && !chkEnableLen.value) return 0; } catch (_) { }

        var v = 0;
        try { v = parseFloat(editVal && editVal.text); } catch (_) { v = 0; }
        if (isNaN(v)) v = 0;
        return v;
    }

    // オブジェクト選択時：元オブジェクトをガイドに変換して残す
    function convertSelectedTargetsToGuides() {
        if (_usingArtboardBase) return;
        try {
            for (var i = 0; i < targetItems.length; i++) {
                var it = targetItems[i];
                if (!it) continue;
                if (_artboardBaseRect && it === _artboardBaseRect) continue;

                var isGuide = false;
                try { isGuide = (it.guides === true); } catch (_) { isGuide = false; }
                if (isGuide) continue;

                try { it.hidden = false; } catch (_) { }
                try { it.guides = true; } catch (_) { }
                // 念のため（ガイド化すると通常は見た目に影響しないが）
                try { it.filled = false; } catch (_) { }
            }
        } catch (_) { }
    }
    ...
    // （この位置にあった重複の createShortenedLines 関数は削除されました）
