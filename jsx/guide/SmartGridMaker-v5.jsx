...
/*
  ...
  ・ダイアログレイアウトを3カラム構成に変更（左：マージン／フレーム、中央：外側エリア／タイトルエリア、右：内側エリア）
  ・フレーム幅を上下左右に分割して指定可能に（UIはマージンと同様の3段組）
  */

/* バージョン / Version */
var SCRIPT_VERSION = "v1.0.19";
...
(function () {
    ...
    colGroup.alignChildren = ["fill", "top"]; // 各カラムを横方向に揃えやすくする
    colGroup.alignment = ["fill", "top"];    // 行自体を横いっぱいに
    colGroup.spacing = 16;
    ...
    // 中央カラム（外側エリア／タイトルエリア）
    var centerCol = colGroup.add("group");
    centerCol.orientation = "column";
    centerCol.alignChildren = ["fill", "top"];
    centerCol.alignment = ["fill", "top"];
    centerCol.spacing = 8;
    ...
    // --- Frame 4-way handles ---
    var editFrameTop, editFrameBottom, editFrameLeft, editFrameRight;
    var chkFrameLink;
    var _syncingFrameWidths = false;
    var applyFrameLinkState;
    ...
    buildOuterUI(centerCol);

    // タイトルエリア
    var titlePanel = centerCol.add("panel", undefined, L("panelTitleBand"));
    ...
    chkFrameEnable.onClick = function () {
        try {
            ...
            if (enabled) {
                // 上が0ならデフォルトで「10」
                var v = parseFloat(editFrameTop.text);
                if (isNaN(v) || v === 0) {
                    editFrameTop.text = "10";
                }
                try { applyFrameLinkState(); } catch (_) { }
    ...
    // フレーム幅入力：3段組（上 / 左＋連動＋右 / 下）
    var frameUnitLabel = getCurrentRulerUnitLabel();

    // 1段目：上（中央寄せ）
    var frameRowTop = framePanel.add("group");
    frameRowTop.orientation = "row";
    frameRowTop.alignChildren = ["center", "center"];
    frameRowTop.alignment = ["fill", "top"];

    var frameTopGroup = frameRowTop.add("group");
    frameTopGroup.orientation = "row";
    frameTopGroup.alignChildren = ["left", "center"];
    frameTopGroup.add("statictext", undefined, L("offsetTop"));
    editFrameTop = frameTopGroup.add("edittext", undefined, "0");
    editFrameTop.characters = 4;
    changeValueByArrowKey(editFrameTop, false);
    frameTopGroup.add("statictext", undefined, frameUnitLabel);

    // 2段目：左 ＋ 連動（中央）＋ 右
    var frameRowMid = framePanel.add("group");
    frameRowMid.orientation = "row";
    frameRowMid.alignChildren = ["center", "center"];
    frameRowMid.alignment = ["fill", "top"];
    frameRowMid.spacing = 12;

    var frameLeftGroup = frameRowMid.add("group");
    frameLeftGroup.orientation = "row";
    frameLeftGroup.alignChildren = ["left", "center"];
    frameLeftGroup.add("statictext", undefined, L("offsetLeft"));
    editFrameLeft = frameLeftGroup.add("edittext", undefined, "0");
    editFrameLeft.characters = 4;
    changeValueByArrowKey(editFrameLeft, false);

    chkFrameLink = frameRowMid.add("checkbox", undefined, L("chkLink"));
    chkFrameLink.value = true;

    var frameRightGroup = frameRowMid.add("group");
    frameRightGroup.orientation = "row";
    frameRightGroup.alignChildren = ["left", "center"];
    frameRightGroup.add("statictext", undefined, L("offsetRight"));
    editFrameRight = frameRightGroup.add("edittext", undefined, "0");
    editFrameRight.characters = 4;
    changeValueByArrowKey(editFrameRight, false);

    // 3段目：下（中央寄せ）
    var frameRowBottom = framePanel.add("group");
    frameRowBottom.orientation = "row";
    frameRowBottom.alignChildren = ["center", "center"];
    frameRowBottom.alignment = ["fill", "top"];

    var frameBottomGroup = frameRowBottom.add("group");
    frameBottomGroup.orientation = "row";
    frameBottomGroup.alignChildren = ["left", "center"];
    frameBottomGroup.add("statictext", undefined, L("offsetBottom"));
    editFrameBottom = frameBottomGroup.add("edittext", undefined, "0");
    editFrameBottom.characters = 4;
    changeValueByArrowKey(editFrameBottom, false);
    frameBottomGroup.add("statictext", undefined, frameUnitLabel);

    // 連動制御
    applyFrameLinkState = function () {
        var linked = !!chkFrameLink.value;
        try { frameBottomGroup.enabled = !linked; } catch (_) { }
        try { frameLeftGroup.enabled = !linked; } catch (_) { }
        try { frameRightGroup.enabled = !linked; } catch (_) { }

        if (linked) {
            var v = editFrameTop.text;
            _syncingFrameWidths = true;
            try { editFrameBottom.text = v; } catch (_) { }
            try { editFrameLeft.text = v; } catch (_) { }
            try { editFrameRight.text = v; } catch (_) { }
            _syncingFrameWidths = false;
        }
    };
    applyFrameLinkState();
    ...
    function applyFrameEnabledState() {
        try {
            ...
            if (!_usingArtboardBase) {
                ...
                try {
                    editFrameTop.text = "0";
                    editFrameBottom.text = "0";
                    editFrameLeft.text = "0";
                    editFrameRight.text = "0";
                    chkFrameLink.value = true;
                    applyFrameLinkState();
                } catch (_) { }
                ...
                // 依存状態の更新
                try { editFrameTop.enabled = false; } catch (_) { }
                try { editFrameBottom.enabled = false; } catch (_) { }
                try { editFrameLeft.enabled = false; } catch (_) { }
                try { editFrameRight.enabled = false; } catch (_) { }
                try { if (chkFrameLink) chkFrameLink.enabled = false; } catch (_) { }
            ...
            // フレーム幅フィールド
            try {
                editFrameTop.enabled = enabled;
                chkFrameLink.enabled = enabled;
                ...
                if (chkFrameLink && chkFrameLink.value) {
                    // 連動ON時は top のみ編集可
                    editFrameBottom.enabled = false;
                    editFrameLeft.enabled = false;
                    editFrameRight.enabled = false;
                } else {
                    editFrameBottom.enabled = enabled;
                    editFrameLeft.enabled = enabled;
                    editFrameRight.enabled = enabled;
                }
            } catch (_) { }

            // valid: 4方向のどれかが >0
            var t = parseFloat(editFrameTop.text);
            var b = parseFloat(editFrameBottom.text);
            var l = parseFloat(editFrameLeft.text);
            var r = parseFloat(editFrameRight.text);
            t = (!isNaN(t) ? t : 0);
            b = (!isNaN(b) ? b : 0);
            l = (!isNaN(l) ? l : 0);
            r = (!isNaN(r) ? r : 0);
            var valid = (t > 0 || b > 0 || l > 0 || r > 0);

            // 裁ち落とし
            if (chkBleed) {
                chkBleed.enabled = (enabled && _usingArtboardBase && valid);
                if (!enabled) chkBleed.value = false;
                if (!valid) chkBleed.value = false;
            }

            // 角丸
            if (chkFrameRound) {
                chkFrameRound.enabled = (enabled && valid);
                if (!enabled) chkFrameRound.value = false;
                if (!valid) chkFrameRound.value = false;

                if (editFrameRound) {
                    editFrameRound.enabled = (enabled && valid && chkFrameRound.value);
                    if (!(enabled && valid)) editFrameRound.text = "0";
                }
            }
        } catch (_) { }
    }
    ...
    (function restoreUIState() {
        ...
        // Frame width (4-way). New nested: st.frame.width.{top,bottom,left,right,link}
        // Fallback from old single value: st.frameWidth / st.frame.width
        safeSet(function () {
            var w = firstDefined(getNested(st, ["frame", "width"]), st.frameWidth, undefined);

            var lk = firstDefined(getNested(st, ["frame", "width", "link"]), st.frameLink, undefined);
            if (typeof lk !== "undefined") chkFrameLink.value = !!lk;

            var t = getNested(st, ["frame", "width", "top"]);
            var b = getNested(st, ["frame", "width", "bottom"]);
            var l = getNested(st, ["frame", "width", "left"]);
            var r = getNested(st, ["frame", "width", "right"]);

            if (typeof t !== "undefined") editFrameTop.text = String(t);
            if (typeof b !== "undefined") editFrameBottom.text = String(b);
            if (typeof l !== "undefined") editFrameLeft.text = String(l);
            if (typeof r !== "undefined") editFrameRight.text = String(r);

            // fallback: single value
            if (typeof t === "undefined" && typeof w !== "undefined" && typeof w !== "object") {
                editFrameTop.text = String(w);
            }
    ...
    // フレーム幅（4-way）
    function onFrameWidthChanged(isTopDriver) {
        ...
        if (_syncingFrameWidths) return;

        if (isTopDriver && chkFrameLink.value) {
            var v = editFrameTop.text;
            _syncingFrameWidths = true;
            try { editFrameBottom.text = v; } catch (_) { }
            try { editFrameLeft.text = v; } catch (_) { }
    ...
    editFrameTop.onChanging = function () { onFrameWidthChanged(true); };
    editFrameBottom.onChanging = function () { onFrameWidthChanged(false); };
    editFrameLeft.onChanging = function () { onFrameWidthChanged(false); };
    editFrameRight.onChanging = function () { onFrameWidthChanged(false); };

    chkFrameLink.onClick = function () {
        applyFrameLinkState();
        applyFrameEnabledState();
        if (chkPreview.value) updatePreview();
    };
...
var _fT = (chkFrameEnable && chkFrameEnable.value) ? parseFloat(editFrameTop.text) : 0;
var _fB = (chkFrameEnable && chkFrameEnable.value) ? parseFloat(editFrameBottom.text) : 0;
var _fL = (chkFrameEnable && chkFrameEnable.value) ? parseFloat(editFrameLeft.text) : 0;
var _fR = (chkFrameEnable && chkFrameEnable.value) ? parseFloat(editFrameRight.text) : 0;
...
var _hasFrame = (chkFrameEnable && chkFrameEnable.value) && (_fT > 0 || _fB > 0 || _fL > 0 || _fR > 0);
...
if ((!isNaN(_okVal) && _okVal <= 0)
    ...
    function collectOptions() {
        ...
        // フレーム幅（pt） 4-way
        var ft = (chkFrameEnable && chkFrameEnable.value) ? parseFloat(editFrameTop.text) : 0;
        var fb = (chkFrameEnable && chkFrameEnable.value) ? parseFloat(editFrameBottom.text) : 0;
        var fl = (chkFrameEnable && chkFrameEnable.value) ? parseFloat(editFrameLeft.text) : 0;
        var fr = (chkFrameEnable && chkFrameEnable.value) ? parseFloat(editFrameRight.text) : 0;

        var frameTopPt = (!isNaN(ft) ? (ft * factor) : 0);
        var frameBottomPt = (!isNaN(fb) ? (fb * factor) : 0);
        var frameLeftPt = (!isNaN(fl) ? (fl * factor) : 0);
        var frameRightPt = (!isNaN(fr) ? (fr * factor) : 0);

        if (frameTopPt < 0) frameTopPt = 0;
        if (frameBottomPt < 0) frameBottomPt = 0;
        if (frameLeftPt < 0) frameLeftPt = 0;
        if (frameRightPt < 0) frameRightPt = 0;

        if (_bleedEnabled) {
            var bleedPtForFrame = (72.0 / 25.4) * BLEED_MM;
            frameTopPt += bleedPtForFrame;
            frameBottomPt += bleedPtForFrame;
            frameLeftPt += bleedPtForFrame;
            frameRightPt += bleedPtForFrame;
        }
        ...
        return {
            ...
            frameTopPt: frameTopPt,
            frameBottomPt: frameBottomPt,
            frameLeftPt: frameLeftPt,
            frameRightPt: frameRightPt,
            offTopPt: offTopPt,
    ...
    function generateFromOptions(opt, isFinal) {
        ...
        if (opt.distPt === 0) {
            // フレーム（アートボードサイズ基準）
            if ((opt.frameTopPt || opt.frameRightPt || opt.frameBottomPt || opt.frameLeftPt)) {
                var abB = getActiveArtboardBounds();
                if (abB) {
                    createFrameFill4(targetItems[0], opt.frameTopPt, opt.frameRightPt, opt.frameBottomPt, opt.frameLeftPt, abB);
                }
            }
        ...
        if ((opt.frameTopPt || opt.frameRightPt || opt.frameBottomPt || opt.frameLeftPt)) {
            ...
            if (abB2) {
                createFrameFill4(targetItems[0], opt.frameTopPt, opt.frameRightPt, opt.frameBottomPt, opt.frameLeftPt, abB2);
            }
        }
    ...
    // 新・フレーム 4-way 幅で作成（ターゲットと同じレイヤーに作る）
function createFrameFill4(target, topPt, rightPt, bottomPt, leftPt, abB) {
    try {
        if (!abB) return;
        if (!(topPt || rightPt || bottomPt || leftPt)) return;

        var L = abB[0], T = abB[1], R = abB[2], B = abB[3];

        // artboardRect: [L, T, R, B]（T > B）
        // 外側へ広げる：Lを左へ、Tを上へ、Rを右へ、Bを下へ
        var oL = L - (leftPt || 0);
        var oT = T + (topPt || 0);
        var oR = R + (rightPt || 0);
        var oB = B - (bottomPt || 0);
        ...
        // 生成先レイヤー：target と同じ（activeLayer がロック等でも確実に作るため）
        var lay = null;
        try { if (target && target.layer) lay = target.layer; } catch (_) { }
        if (!lay) lay = doc.activeLayer;

        var p = null;
        try {
            p = lay.pathItems.rectangle(oT, oL, w, h);
        } catch (_) {
            try { p = doc.activeLayer.pathItems.rectangle(oT, oL, w, h); } catch (__e) { p = null; }
        }
        if (!p) return;

        // createFrameFill と同じ扱い（タグは __FrameFill__）
        try { p.stroked = false; } catch (_) { }
        try { p.filled = true; } catch (_) { }
        try { p.fillColor = makeK15Fill(); } catch (_) { }

        try { p.note = "__FrameFill__"; } catch (_) { }
        try { p.name = "__FrameFill__"; } catch (_) { }
