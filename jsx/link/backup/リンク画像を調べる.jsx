...
(function () {
    ...
    /*
        ...
        一覧表示・検索・整理・再リンクを効率化するユーティリティ。

         Illustrator標準機能では困難な
        「全体把握・重複管理・再リンク作業」を一元化することを目的とする。
    
        ### 主な機能
    
        ・ファイル名／サイズ／寸法／PPI／拡大率の一覧表示
        ・配置先アートボードの特定と複数配置の可視化
        ・同一ファイルの重複検出・統合表示・使用数カウント
        ・任意列でのソート（昇順／降順）
        ・リンク状態（正常／切れ／要更新）のフィルタリング
        ・アートボード単位での絞り込み
        ・行選択に連動したカンバス上での選択・ズーム表示
        ・パス表示の最適化（~/ 表示／Dropbox 短縮）
        ・リンク先フォルダの即時オープン
        ・単一ファイル／フォルダ単位での再リンク
    ...
    */
    ...
    function readImagePixelSize(file) {
        if (!file) return null;
        try { if (!file.exists) return null; } catch (e) { return null; }
    ...
    // 「アップデートが必要」判定。
    // ExtendScript から「配置時に保持されていたタイムスタンプ」を確実に取得する手段がなく、
    // ドキュメント保存時刻との比較では誤検知が多かったため、現時点では常に false を返す。
    // （必要になったら doc.XMPString を解析して stRef:lastModifyDate と比較する実装に差し替える）
    function isLinkOutOfDate(item, doc) {
        return false;
    }
    ...
    function getScaleInfo(item) {
        try {
            ...
            return { scalePct: null, scaleText: "---" };
        }
    }
    ...
    function getDimensionsMm(item) {
        ...
        } catch (e) { return { widthMm: null, heightMm: null }; }
    }
    ...
    function collectLinkInfo(doc, placedItems) {
        ...
        for (var i = 0; i < placedItems.length; i++) {
            ...
            var f = null;
            try { f = item.file; } catch (e) { f = null; }

            var isLinkOk = false;
            var statusCode = "broken";

            if (f) {
                try { if (f.name) fileName = f.name; } catch (e) { }
                try { if (f.fsName) filePath = f.fsName; } catch (e) { }
                try {
                    if (f.exists) {
                        if (isLinkOutOfDate(item, doc)) {
                            statusCode = "update";
            ...
            var statusIcon = statusCode === "ok" ? "✓"
                : statusCode === "update" ? "⟳"
                    : "⚠";

            if (fileName === L('fileNameUnknown')) {
                try {
                    if (item.name && item.name !== "") fileName = item.name;
                } catch (e) { }
            }
            ...
            var pxSize = null;
            if (f) {
                try { pxSize = readImagePixelSize(f); } catch (e) { pxSize = null; }
    ...
    function showLinkDialog(allPlacementEntries, uniqueFileEntries, placedItems, doc, preselectedItemIndex) {
        var PANEL_MARGINS = [15, 20, 15, 10]; // 全パネル共通のマージン
        ...
        // --- 上部 3 カラム：（ソート + オプション） / 表示オプション / （フィルター + アートボード） ---
        ...
        // 中央カラム：表示オプション（列の表示/非表示を切替）
        var optionPanel = topRow.add("panel", undefined, L('displayOptionPanelTitle'));
        ...
        var artboardCheck = optionPanel.add("checkbox", undefined, L('displayArtboard'));
        sizeColCheck.value = true;
        countColCheck.value = true;
        dimScalePpiCheck.value = false;
        artboardCheck.value = true;

        // 右カラム：フィルター + アートボード を縦に
        var middleCol = topRow.add("group");
        middleCol.orientation = "column";
        middleCol.alignChildren = ["fill", "top"];

        var filterPanel = middleCol.add("panel", undefined, L('filterPanelTitle'));
        ...
        // アートボードパネル：先頭に「すべて」、続けて「番号：名前」
        var abPanel = middleCol.add("panel", undefined, L('artboardPanelTitle'));
        abPanel.orientation = "row";
        abPanel.alignChildren = ["left", "center"];
        abPanel.alignment = ["left", "top"]; // パネル自体を縮めて右側のアキを消す
        ...
        for (var artboardIndex = 0; artboardIndex < doc.artboards.length; artboardIndex++) {
            var artboardName = "";
            try { artboardName = doc.artboards[artboardIndex].name; } catch (e) { }
        ...
        abFilterDropdown.preferredSize.width = 140;
        ...
        function getColumnSpec() {
            var cols = [];
            cols.push({ key: "index", title: L('colIndex'), width: 40 });
            cols.push({ key: "fileName", title: L('colFileName'), width: 180 });
        ...
        function rebuildList() {
            ...
            for (var j = 0; j < filteredEntries.length; j++) {
                ...
                // 先頭列（No.）は .text、残りは subItems にセット
                var row = listBox.add("item", String(info.index));
        ...
        function applyArtboardFilter() {
            ...
            // 既存のフィルタロジック
            if (selectedArtboardIndex > 0 && artboardCheck.value) {
                artboardCheck.value = false;
                createListBox();
        ...
        artboardCheck.onClick = recreateListBoxAndRebuildList;
        ...
        foldersListBox.preferredSize = [500, 100];
        ...
        // --- リンクフォルダ一覧の直下：選択フォルダ配下のリンク一括差し替えボタン ---
        var folderActionRow = pathPanel.add("group");
        folderActionRow.orientation = "row";
        folderActionRow.alignChildren = ["left", "center"];

        var reloadFolderBtn = folderActionRow.add("button", undefined, L('reloadFolderBtn'));
