...
/*
  MaskFitter Clone Script (Auto-Preview)
  更新日: 2026-01-02

  - ショートカット: qwe / asd / zxc
  - 数値増減: ↑↓(±1), Shift+↑↓(±10), Opt+↑↓(±0.1)
  - 角丸エフェクトを「クリップグループ全体」に適用
...
/action-1 {
    ...
    radioCover.value = true;
    changeValueByArrowKey(scaleInput, function () { if (radioManual.value) updatePreview(); });
    ...
    changeValueByArrowKey(roundInput, function () { if (checkRound.value) updatePreview(); });
    // ボタン
    ...
    function setRoundRadiusToHalfOfSquareMask() {
        try {
            ...
            // 半径＝幅または高さの半分（正方形なら同値）
            var radiusPt = Math.max(w, h) / 2;
    ...
    scaleInput.onChanging = function () { if (radioManual.value) updatePreview(); };
    roundInput.onChanging = function () { if (checkRound.value) updatePreview(); };
    for (var i = 0; i < anchorRadios.length; i++) {
        (function (idx) {
