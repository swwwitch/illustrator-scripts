...
画像またはベクターオブジェクトのグループと、パスを選択して実行すると、アートワークの複製をパスでマスクして、すりガラス風のぼかし・ルーペ・部分拡大を作ります。

詳細は README を参照してください。

### Overview

With an image or a group of vector objects and a path selected, masks a copy of the artwork with the path to build a frosted-glass panel, a loupe or a detail callout.
...
(function() {
    ...
    /* ぼかす範囲の種類 / Blur area modes */
    var BLUR_AREA_NONE    = "none";    /* ぼかさない / no blur */
    var BLUR_AREA_INSIDE  = "inside";  /* マスク内をぼかす / blur inside the mask */
    var BLUR_AREA_OUTSIDE = "outside"; /* マスク外をぼかす / blur outside the mask */
    ...
    var PRESETS = [
        {
            ...
            blurArea: BLUR_AREA_INSIDE, blurRadius: 10,
        ...
        },
        {
            ...
            blurArea: BLUR_AREA_OUTSIDE, blurRadius: 15,
            addsStroke: false, addsDropShadow: false, addsConnector: false,
            shadowOpacity: DEFAULT_SHADOW_OPACITY, shadowOffsetX: DEFAULT_SHADOW_OFFSET_X, shadowOffsetY: DEFAULT_SHADOW_OFFSET_Y,
            shadowBlur: DEFAULT_SHADOW_BLUR, shadowDarkness: DEFAULT_SHADOW_DARKNESS,
            addsLoupe: false, duplicatesLoupe: false, scalesLoupeByEffect: false,
        ...
        },
        {
            ...
            blurArea: BLUR_AREA_NONE, blurRadius: 10,
        ...
        {
            /* 拡大コピーをすぐ右下へ置く（100%＝マスク1つ分ずらす）
               「効果で拡大」で使う変形効果は上方向が正なので、下へ動かすYは負の値にする
               A magnified copy set just to the lower right, one mask size across;
               the Transform effect counts up as positive, so moving down takes a negative Y */
            labelKey: "preset.callout",
            blurArea: BLUR_AREA_NONE, blurRadius: 15,
            addsStroke: true, addsDropShadow: true, addsConnector: false,
            shadowOpacity: DEFAULT_SHADOW_OPACITY, shadowOffsetX: DEFAULT_SHADOW_OFFSET_X, shadowOffsetY: DEFAULT_SHADOW_OFFSET_Y,
            shadowBlur: DEFAULT_SHADOW_BLUR, shadowDarkness: DEFAULT_SHADOW_DARKNESS,
            addsLoupe: true, duplicatesLoupe: true, scalesLoupeByEffect: true,
            loupeScale: 140, loupeOffsetX: 100, loupeOffsetY: -100
        },
        {
    ...
    // =========================================
    var NUMBER_LABEL_WIDTH = 64;  /* 数値欄のラベル幅（px） / label width of a number row */
    var SLIDER_LABEL_WIDTH = 22;  /* スライダー行のラベル幅（px） / label width of a slider row */
    ...
    var LABELS = {
        dialog: {
            title: { ja: "ルーペとすりガラス", en: "Loupe and Frosted Glass" }
        },
        panel: {
            blurArea: { ja: "ぼかす範囲", en: "Blur area" },
        ...
        preset: {
            foreground: { ja: "前面をぼかす", en: "Blurred foreground" },
            background: { ja: "背景をぼかす", en: "Blurred background" },
            loupe:      { ja: "ルーペ", en: "Loupe" },
            callout:        { ja: "部分拡大", en: "Detail callout" },
            calloutBlurred: { ja: "部分拡大（背景をぼかす）", en: "Detail callout, blurred background" },
            maskWithShadow: { ja: "マスクのみ＋ドロップシャドウ", en: "Mask with drop shadow" },
            custom:     { ja: "カスタム", en: "Custom" },
            saved:      { ja: "保存した設定", en: "Saved settings" }
        },
        radio: {
            blurNone:    { ja: "なし", en: "None" },
            blurInside:  { ja: "マスク内", en: "Inside the mask" },
            blurOutside: { ja: "マスク外", en: "Outside the mask" }
        },
        checkbox: {
            addStroke:    { ja: "境界線を追加", en: "Add stroke" },
            addConnector: { ja: "拡大表示のコネクター", en: "Zoom connector" },
            dropShadow:   { ja: "ドロップシャドウを追加", en: "Add drop shadow" },
            addLoupe:       { ja: "ルーペを追加", en: "Add loupe" },
            duplicateLoupe: { ja: "複製", en: "Duplicate" },
            scaleByEffect:  { ja: "効果で拡大", en: "Scale as an effect" },
        ...
        tooltip: {
            ...
            blurRadius:  { ja: "ぼかし（ガウス）の半径（px）です。", en: "Radius of the Gaussian blur, in pixels." },
            blurNone:    { ja: "ぼかしません。マスクを作るだけで、境界線やドロップシャドウは使えます。", en: "Applies no blur. It only makes the mask; the stroke and drop shadow still apply." },
            blurInside:  { ja: "マスクの内側をぼかします。背景はそのまま残ります。", en: "Blurs the inside of the mask and leaves the background sharp." },
            ...
            addConnector: { ja: "元の位置と拡大したコピーを線でつなぐグラフィックスタイルを適用します。ルーペを複製する「部分拡大」のときだけ使えます。", en: "Applies a graphic style that links the artwork and the magnified copy with lines. Available only where the loupe is duplicated, as in the Detail callout presets." },
            ...
            addLoupe:     { ja: "マスクを拡大して、拡大鏡のように重ねます。ぼかす範囲が「なし」「マスク外」のときに使えます。", en: "Scales the mask up and lays it over the artwork like a magnifier. Available when the blur area is None or Outside the mask." },
            duplicateLoupe: { ja: "元のガラス部分を残したままルーペを増やします。「効果で拡大」がオンなら、実体は増やさず［変形］効果のコピーで作ります。", en: "Keeps the original glass panel and adds the loupe beside it. With Scale as an effect on, the copy comes from the Transform effect instead of a real duplicate." },
    ...
    /**
     * ラベルを取得する（ドット区切りキー）
     * @param {string} labelPath - "panel.blurArea" のようなドット区切りキー
    ...
    var SETTING_NUMBER_KEYS = ["blurRadius", "shadowOpacity", "shadowOffsetX", "shadowOffsetY",
    ...
    function saveSettingsToFile(settings) {
        try {
            ...
            var settingLines = ["blurArea=" + settings.blurArea];
            var i;
    ...
    function loadSavedPreset() {
        ...
        if (!storedValues.blurArea) return null;

        var savedPreset = { labelKey: "preset.saved", blurArea: storedValues.blurArea };
        var j;
        for (j = 0; j < SETTING_NUMBER_KEYS.length; j++) {
            savedPreset[SETTING_NUMBER_KEYS[j]] = Number(storedValues[SETTING_NUMBER_KEYS[j]]);
        }
     ...
     * 「効果で拡大」がオンなら［変形］効果だけで済ませ、「複製」は効果のコピーで作る
    ...
    function applyLoupe(clipGroup, settings) {
        ...
        /* 効果で拡大するときは、複製も［変形］効果のコピー（numCopies）で作る
    ...
    /**
     * マスクした複製を作り、ぼかし・境界線・ドロップシャドウ・ルーペを適用する
     * 確定実行では元のアイテムとパスをそのまま使うため、増えるのは複製1つとクリップグループだけ
     * @param {object} settings - blurArea、blurRadius、addsStroke、addsDropShadow、ルーペ関連を持つ設定
     * @param {boolean} isCommitRun - true で確定実行、false でプレビュー
     * @returns {object} 作成したクリップグループ
     */
    function buildSpotlight(settings, isCommitRun) {
        ...
        if (settings.blurRadius > 0) {
            if (settings.blurArea === BLUR_AREA_INSIDE) {
                applyGaussianBlur(maskedArtworkCopy, settings.blurRadius);
            } else if (settings.blurArea === BLUR_AREA_OUTSIDE) {
        ...
        var loupeGroup = null;
        if (settings.addsLoupe && settings.blurArea !== BLUR_AREA_INSIDE) {
     ...
     * @param {object} settings - blurArea、addsLoupe、duplicatesLoupe を持つ設定
     * @returns {boolean} 使えるなら true
     */
    function canAddConnector(settings) {
        return settings.addsLoupe && settings.duplicatesLoupe && settings.blurArea !== BLUR_AREA_INSIDE;
    }
    ...
    function createPresetRow(parentWindow, presetList) {
        ...
        presetRow.add('statictext', undefined, labelText('label.preset'));
     ...
     * 「ぼかす範囲」パネルを組み立てる
     * @param {object} parentWindow - パネルを追加する親
     * @returns {object} 各ラジオと半径欄、半径の入れ物を持つオブジェクト
     */
    function createBlurAreaPanel(parentWindow) {
        var blurAreaPanel = parentWindow.add('panel', undefined, getLabel('panel.blurArea'));
        blurAreaPanel.orientation = 'column';
        blurAreaPanel.alignChildren = ['left', 'top'];
        blurAreaPanel.margins = [15, 20, 15, 10];

        /* ラジオは同じ親に入れないと排他にならない / radios must share a parent to stay exclusive */
        var blurAreaRadioRow = blurAreaPanel.add('group');
        blurAreaRadioRow.orientation = 'row';
        blurAreaRadioRow.alignChildren = ['left', 'center'];

        var blurNoneRadio = blurAreaRadioRow.add('radiobutton', undefined, getLabel('radio.blurNone'));
        blurNoneRadio.helpTip = getLabel('tooltip.blurNone');

        var blurInsideRadio = blurAreaRadioRow.add('radiobutton', undefined, getLabel('radio.blurInside'));
        blurInsideRadio.helpTip = getLabel('tooltip.blurInside');
        blurInsideRadio.value = true;

        var blurOutsideRadio = blurAreaRadioRow.add('radiobutton', undefined, getLabel('radio.blurOutside'));
        blurOutsideRadio.helpTip = getLabel('tooltip.blurOutside');

        /* 「なし」のときにラベルごとディム表示にするための入れ物
           A container so the label dims together with the field when no blur is applied */
        var blurRadiusGroup = blurAreaPanel.add('group');
        blurRadiusGroup.orientation = 'column';
        blurRadiusGroup.alignChildren = ['fill', 'top'];

        return {
            noneRadio: blurNoneRadio,
            insideRadio: blurInsideRadio,
            outsideRadio: blurOutsideRadio,
            radiusGroup: blurRadiusGroup,
            radiusField: createNumberFieldRow(blurRadiusGroup, 'label.blurRadius', 'tooltip.blurRadius', DEFAULT_BLUR_RADIUS, 'px')
        };
    }
     ...
     * @param {Array} presetList - ドロップダウンに並べるプリセット
     * @returns {object} preset、blurArea、option、dropShadow、loupe、buttonRow を持つオブジェクト
     */
    function createDialogContents(dialog, presetList) {
        var presetControls = createPresetRow(dialog, presetList);

        /* 上段：ぼかす範囲とオプション / Top row: blur area and options */
        ...
        var blurAreaControls = createBlurAreaPanel(topPanelRow);
        var optionControls = createOptionsPanel(topPanelRow);

        /* 下段：ドロップシャドウとルーペ / Bottom row: drop shadow and loupe */
    ...
    function showSettingsDialog() {
        ...
        var blurAreaControls = dialogControls.blurArea;
        ...
        /**
         * 選択中のラジオからぼかす範囲を読み取る
         * @returns {string} BLUR_AREA_NONE / BLUR_AREA_INSIDE / BLUR_AREA_OUTSIDE のいずれか
         */
        function getBlurAreaMode() {
            if (blurAreaControls.noneRadio.value) return BLUR_AREA_NONE;
            if (blurAreaControls.outsideRadio.value) return BLUR_AREA_OUTSIDE;
            return BLUR_AREA_INSIDE;
        }

        /* 数値欄と、その許容範囲。範囲はここだけに書く
           Every number field with its range, declared in one place */
        var numberFieldSpecs = {
            blurRadius:     { field: blurAreaControls.radiusField,     min: 0,                  max: MAX_BLUR_RADIUS },
         ...
         * 半径は「なし」以外、ルーペはマスク内をぼかすとき以外、コネクターは「部分拡大」のときだけ使える
         * @returns {void}
         */
        function updateControlStates() {
            var blurAreaMode = getBlurAreaMode();
            blurAreaControls.radiusGroup.enabled = (blurAreaMode !== BLUR_AREA_NONE);
            dropShadowControls.settingsGroup.enabled = dropShadowControls.addCheckbox.value;
            loupeControls.panel.enabled = (blurAreaMode !== BLUR_AREA_INSIDE);
        ...
        function applyPreset(preset) {
            blurAreaControls.noneRadio.value = (preset.blurArea === BLUR_AREA_NONE);
            blurAreaControls.insideRadio.value = (preset.blurArea === BLUR_AREA_INSIDE);
            blurAreaControls.outsideRadio.value = (preset.blurArea === BLUR_AREA_OUTSIDE);
        ...
        blurAreaControls.noneRadio.onClick = onSettingChanged;
        blurAreaControls.insideRadio.onClick = onSettingChanged;
        blurAreaControls.outsideRadio.onClick = onSettingChanged;
