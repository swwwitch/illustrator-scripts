...
https://note.com/nice_lotus120/n/nf406fb3ae2b4
  ...
  v1.0.1: スムーズポイント化が直角にも影響するため、処理を削除。
...
(function () {
        ...
        try {
            ...
            for (var i = items.length - 1; i >= 0; i--) {
                try {
                    ...
                    // 念のためオリジナルは消さない
                    if (it === originalPath) continue;
                    var nm = "";
                    try { nm = it.name; } catch (_) { nm = ""; }
    ...
    function buildMergedItemFromGroup(groupItem) {
        ...
        try {
            ...
            try { if (merged && merged.isValid) merged.name = TEMP_BASE_NAME; } catch (_) { }
            res.item = merged;
            res.ok = true;
            return res;
    ...
    function doExtrude(applyOffset) {
        ...
        try {
            ...
            if (baseIsTemp) {
                try { baseCleanup(); } catch (_) { }
                // 参照切れや一部削除失敗に備えて、名前で残骸も回収
                try { removeAllTempBasesByName(); } catch (_) { }
                // 最後の保険：baseItem 自体が残っていれば削除
                try {
                    if (baseItem && baseItem.isValid && baseItem !== originalPath) {
                        var nm2 = "";
                        try { nm2 = baseItem.name; } catch (_) { nm2 = ""; }
                        if (nm2 === TEMP_BASE_NAME) baseItem.remove();
                    }
                } catch (_) { }
            }
        }
