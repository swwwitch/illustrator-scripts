...
/*
  ...
  対象：閉パスの PathItem / CompoundPathItem（複合パス）
...
(function () {
    ...
    function doExtrude(applyOffset) {
        ...
        // --- プレビュー簡易化 ---
        // プレビュー時は「合体した影」を作らず、複製オブジェクト（pathB）のみ表示する
        if (!__applyOffsetNow) {
            ...
            // 中間表示（scale=100%でも表示する）
            // 生成順：薄い→濃い（手前が見やすい）
            try { pathB.move(originalPath, ElementPlacement.PLACEBEFORE); } catch (_) { }
            try { pathB.opacity = 40; } catch (_) { }
