...
・読み込み：ファイル未指定時は［読み込みファイル］パネル以外をディム表示。アートボード番号（例 1-20 / 1,3,5）を指定して配置
...
function main() {
    ...
    function setPathText(f) {
        ...
        __SC_updateSourceDependentPanels();
    }
    ...
    function __SC_updateSourceDependentPanels() {
        var hasFile = !!fileA;

        try { pnlAB.enabled = hasFile; } catch (_) { }
        try { panelCrop.enabled = hasFile; } catch (_) { }
        try { panelLayout.enabled = hasFile; } catch (_) { }
        try { panelEven.enabled = hasFile; } catch (_) { }
        try { panelLayoutRight.enabled = hasFile; } catch (_) { }
        try { panelArtboard.enabled = hasFile; } catch (_) { }
    }
