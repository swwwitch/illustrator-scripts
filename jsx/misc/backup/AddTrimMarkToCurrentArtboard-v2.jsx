...
- 選択オブジェクト（単純な長方形1点）または現在のアートボードを対象に、トンボを作成するIllustrator用スクリプトです。
...
- トンボは専用の「トンボ」レイヤー上に作成され、処理後はレイヤーをロックします。
- ダイアログのUIおよびラベルは日本語／英語に自動対応します。

### 主な機能

- 選択オブジェクト（単純な長方形1点）または現在のアートボードを対象として選択可能
- 実行時ダイアログで「ガイドを残す」「日本式トンボ」のON/OFFを切り替え
- 「トンボ」レイヤーを自動取得／未存在時は新規作成
- 単純な長方形1点、またはアートボード矩形1つを対象に処理
...
6. その対象オブジェクトを元にトリムマーク作成メニューを実行
7. 「ガイドを残す」がONなら同じ対象オブジェクトをガイド化
8. 「ガイドを残す」がOFFなら同じ対象オブジェクトを削除
...
function showOptionsDialog() {
    ...
    return {
        targetType: rdoSelection.value ? 'selection' : 'artboard',
...
function main() {
    ...
    // レイヤーのロック状態を保存してからアンロック / Store lock state and unlock temporarily
    ...
    try {
        ...
        /* 作成オブジェクトを選択する / Select the created object */
        ...
        if (options.createGuide) {
            ...
            /* 作成オブジェクトを削除する / Remove the created object */
            targetObj.remove();
            targetObj = null;
        }
    } finally {
