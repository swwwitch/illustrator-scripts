    ...
    try {
        for (var p = 0; p < targetPaths.length; p++) {
            ...
            for (var j = 0; j < pairs.length; j++) {
                ...
                drawLineAcrossArtboard(lineGroup, p1, p2, abLeft, abTop, abRight, abBottom, shouldGuide, shouldDedup, dedupMap);
            }
        }
    } finally {
...
function showDialog(countAll, countStraight) {
    ...
    // Make mode options mutually exclusive (ScriptUI radiobuttons only auto-exclusive within the same parent)
    rbAll.onClick = function () {
        rbStraight.value = false;
    };
    rbStraight.onClick = function () {
        rbAll.value = false;
    };
...
function drawLineAcrossArtboard(targetGroup, p1, p2, left, top, right, bottom, shouldGuide, shouldDedup, dedupMap) {
    ...
    if (intersections.length >= 2) {
        ...
        if (uniquePoints.length === 2) {
            ...
            if (shouldGuide) {
                newLine.stroked = false;
                try { newLine.guides = true; } catch (_) { }
            } else {
                ...
                newLine.strokeWidth = (0.1 * 72.0) / 25.4;
            }
        }
    }
}
