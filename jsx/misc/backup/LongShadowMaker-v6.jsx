...
/*
  ...
  ・マド処理（チェックボックス ON 時）：Pathfinder Merge → Add → Expand により影の穴を解消
  ・マド処理 OFF 時：従来どおり Add（必要箇所のみ）で処理し、形状を保持
...
2026-02-09: 生成したシャドウの穴を潰すため Pathfinder Merge→Add→Expand を追加
2026-02-09: テキスト実行時に生成した影が消えることがある問題を修正
 - TextFrame のアウトライン化一時生成物の削除処理を見直し、影パスが消える（実際に消失が確認された）問題を修正
2026-02-09: Text/Group の一時ベース（②）が残ることがある問題を修正（名前タグで残骸を回収して削除）
2026-02-09: 一時ベース回収処理のループが構文エラーで止まる問題を修正。
2026-02-09: 未使用関数（convexHull/createHullPath/isCurvedPath/smoothPolylineAsCurve/executeNoCompoundAndAddExpand/applyFillColorToItem/sampleClosedPath/sampleClosedItem）を整理
2026-02-09: スケールが1%のとき、実行時は0.01%として扱うように変更
2026-02-09: 最終処理で __LongShadowTempBase__ レイヤー（残っていれば）を削除
 更新日: 2026-02-09
*/
