...
(function () {
    ...
    function refreshPreview() {
        ...
        try {
            buildCalendar(doc, base, PREVIEW_LAYER_NAME, fs, align, monthTitleAlign, fontName, sundayRed, holidayRed, ghost,
    ...
    okBtn.onClick = function () {
        ...
        try {
            buildCalendar(doc, baseDate, PREVIEW_LAYER_NAME, fs, align, monthTitleAlign, fontName, sundayRed, holidayRed, ghost,
                cellW, cellH, cellGapX, cellGapY, cellPosAdjY, cellFill,
                strokeMode, strokeTop, strokeBottom, strokeLeft, strokeRight,
                showTopYear, includeYearInMonthTitle, monthTitleBottomBorder, topYearBottomMargin,
                monthCount, colCount, monthTitleMode, monthBottomMargin, weekdayBottomMargin,
                outerMarginX, outerMarginY, startFromJanuary, weekStartMonday,
                weekdayLabelMode, weekdayFontSize, monthFontSize, yearFontSize);
        } catch (e) {
        ...
        for (var bi = 0; bi < monthInfos.length; bi++) {
            ...
            for (var idx = 0; idx < totalCells; idx++) {
                ...
                if (!rbStrokeNone.value) {
                    ...
                    var strokeWidth = 0.3; // 固定（必要ならUI化可能）
                    var topY = yy + cellH;   // 上辺
                    var bottomY = yy;        // 下辺
                    var leftX = x;
                    var rightX = x + cellW;
                    ...
                    function drawLine(x1, y1, x2, y2) {
                        try {
                            ...
                            if (monthGroup) {
                                try { line.move(monthGroup, ElementPlacement.PLACEATEND); } catch (_) { }
                            }
                        } catch (_) { }
                    }

                    if (rbStrokeAll.value) {
                        drawLine(leftX, topY, rightX, topY);       // 上
                        drawLine(leftX, bottomY, rightX, bottomY); // 下
                        drawLine(leftX, bottomY, leftX, topY);     // 左
                        drawLine(rightX, bottomY, rightX, topY);   // 右
                    }

                    if (rbStrokeCustom.value) {
                        if (chkStrokeTop.value) drawLine(leftX, topY, rightX, topY);
                        if (chkStrokeBottom.value) drawLine(leftX, bottomY, rightX, bottomY);
                        if (chkStrokeLeft.value) drawLine(leftX, bottomY, leftX, topY);
                        if (chkStrokeRight.value) drawLine(rightX, bottomY, rightX, topY);
                    }
                }
                ...
                var cellTop = yy + __bgAdjY; // 背景だけ上下調整
