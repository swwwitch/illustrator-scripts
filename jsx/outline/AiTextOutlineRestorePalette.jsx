...
(function () {
    ...
    /**
     * 和文の文言を、対応表を使って現在のロケールの表記へ変換する
     * @param {Object} table - 和文表記をキーにした対応表
     * @param {string} jaText - 変換元の和文表記
    ...
    /**
     * 関数のソース文字列から、宣言行〜閉じ括弧行だけを切り出す
     *
     * Function.toString() が周辺コメントを含めて返すことがあるため、BridgeTalkへ送る前に本体だけを抜き出す。
     * @param {string} rawSource - 関数のソース文字列
     * @returns {string} 関数宣言だけを取り出したソース
     */
    ...
    /**
     * ワーカー関数群のソースを連結し、末尾に呼び出し式を付けて返す
     *
     * 同じ関数配列に対しては組み立て結果をキャッシュする。
     * @param {Array<Function>} funcs - 同梱するワーカー関数の配列
     * @param {string} entryCall - 末尾に付ける呼び出し式
    ...
    /**
     * BridgeTalkでメインエンジンへ処理を委譲し、結果を同期的に受け取る
     * @param {string} entryCall - メインエンジンで実行する呼び出し式
     * @param {Array<Function>} funcs - 同梱するワーカー関数。省略時は WORKER_FUNCS
     * @returns {string} ワーカーが返した文字列。失敗時は "ERR:" で始まる文字列
     */
     ...
     * @param {Window} win - 対象のパレット
     * @param {string} result - ワーカーの戻り値
     * @param {string} doneKey - 成功時に表示する文言のキー
     * @returns {void}
     */
    ...
    /**
     * 真偽値を、ワーカーへ渡す引数用の文字列に変換する
     * @param {boolean} value - 変換する値
    ...
    /**
     * ワーカーを呼び出し、送信自体が失敗した場合もエラー文字列に畳んで返す
     *
     * 例外で処理が抜けて isBusy が立ったままになるのを防ぐ。
     * @param {string} entryCall - メインエンジンで実行する呼び出し式
     * @param {Array<Function>} funcs - 同梱するワーカー関数
     * @returns {string} ワーカーの戻り値、または "ERR:" で始まる文字列
     */
    ...
    /**
     * 再入を防ぎながらワーカーを呼び出し、結果をステータスへ反映する
     * @param {Window} win - 対象のパレット
     * @param {string} entryCall - メインエンジンで実行する呼び出し式
     * @param {string} doneKey - 成功時に表示する文言のキー
     * @returns {string} ワーカーの戻り値。処理中だった場合は null
     */
     ...
     * ［復元］ボタンの処理を実行する
     *
     * 復元の直前にメモを読み直して表示し、チェックボックスの状態をワーカーへ渡す。
     * @param {Window} win - 対象のパレット
     * @returns {void}
     */
    ...
    /**
     * 整形済みのメモ文字列を、項目と値の2列リストへ流し込む
     * @param {Window} win - 対象のパレット
     * @param {string} formattedNote - 「項目： 値」形式の行を並べた文字列
     * @returns {void}
     */
    ...
    /**
     * 選択オブジェクトのメモを読み直して、リストへ表示する
     * @param {Window} win - 対象のパレット
     * @param {boolean} keepStatus - true のときはステータス表示を書き換えない
     * @returns {void}
     */
    ...
    /**
     * ヘルプチップ付きのボタンをパネルへ追加する
     * @param {Group} parent - 追加先のグループ
     * @param {string} labelKey - ラベルの文言キー
     * @param {string} tipKey - ヘルプチップの文言キー
     * @param {Function} handler - onClick に割り当てる処理
    ...
    /**
     * ヘルプチップ付きのパネルをウィンドウへ追加する
     * @param {Window} win - 追加先のウィンドウ
     * @param {string} labelKey - ラベルの文言キー
     * @param {string} tipKey - ヘルプチップの文言キー
     ...
     * パレットを組み立てて表示する
     *
     * 多重起動を防ぐため、既存のパレットがあれば閉じてから作り直す。
     * @returns {void}
     */
