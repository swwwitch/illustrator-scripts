...
function main() {
    ...
    try {
        ...
        function collectUIValues() {
            var linked = chkLR.value && chkLinkCenter.value && rdDotC.value;

            return {
                // L/R perforation
                strokeWValueLR: linked ? parseFloat(inputWidthC.text) : parseFloat(inputWidthLR.text),
                gapValueLR: linked ? parseFloat(inputGapC.text) : parseFloat(inputGapLR.text),
                dashLenValueLR: linked ? parseFloat(inputDashLenC.text) : parseFloat(inputDashLenLR.text),
                lrLinked: linked,
        ...
        function applyEffect(isPreview) {
            ...
            var lrStrokeFactor = ui.lrLinked ? strokePtFactor : rulerPtFactor;
            var strokeWLR = toPt(ui.strokeWValueLR, lrStrokeFactor);
            ...
            for (var i = 0; i < targets.length; i++) {
                ...
                if (useWRound) {
                    applyWRoundEdge(doc, rect, items, x, y, w, h, lineX, ui.edgeSizeValueC, ui.cornerSizeValue);
                }
        ...
        inputDashLenC.onChanging = function () { syncLRFromCenter(); updatePreview(); };
        ...
        function updateEdgeDimC() {
            var onEdge = chkCenter.value && !rdEdgeNoneC.value;
            var onWRound = chkCenter.value && chkEdgeWRoundC.value;
            var on = onEdge || onWRound;
            grpEdgeRadioC.enabled = !onWRound;
        ...
        function updateCornerDim() {
            var wRoundOn = chkEdgeWRoundC.value;
            pnlCorner.enabled = !wRoundOn;
            grpCornerSize.enabled = !wRoundOn && !rdCornerNone.value;
        }
        updateCornerDim();
