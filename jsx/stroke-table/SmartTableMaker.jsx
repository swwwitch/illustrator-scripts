...
(function () {
    ...
    // プレビュー時にUndo履歴を汚さないための簡易マネージャ
    // - プレビュー更新前に rollback() で元に戻す
    // - OK時は confirm(finalAction) で一度戻してから本番を1回だけ実行
    // ==============================
    function PreviewManager() {
        ...
        function getHistoryLen() {
            try {
                return app.activeDocument && app.activeDocument.historyStates
                    ? app.activeDocument.historyStates.length
                    : 0;
            } catch (e) {
                return 0;
            }
        ...
        this.addStep = function (func) {
            var beforeLen = getHistoryLen();
            try {
                func();
                var afterLen = getHistoryLen();
                var delta = afterLen - beforeLen;
                if (delta < 1) delta = 1; // 保険
                this.undoDepth += delta;
                safeRedraw();
            } catch (e) {
                // 失敗時は可能な限り巻き戻す
                try { this.rollback(); } catch (e2) { }
        ...
        this.confirm = function (finalAction) {
            if (typeof finalAction === "function") {
                // 一度プレビューを完全に戻してから、本番を1回だけ実行
                this.rollback();
                finalAction();
                // 本番は「現在の状態」なので depth は 0 のままでOK
                this.undoDepth = 0;
                safeRedraw();
            } else {
                // プレビュー状態をそのまま採用
    ...
    function clearPreviewRects() {
        // Undoベースでまず巻き戻す（履歴を汚さない）
        try { previewMgr.rollback(); } catch (eUndo) { }

        // 念のため、マーク済みプレビュー図形をドキュメント全体から掃除（保険）
        try { removeMarkedPreviewItems(doc); } catch (eMark) { }
    ...
    function showHeightDialog(defaultPercent, previewFn, clearPreviewFn) {
        ...
        function applyPreview() {
            ...
            // 常に「最新のUI値」をキャッシュ（Preview OFFでもOK時に使う）
            lastPreviewPercent = v;
            lastOverallFrame = !!cbOverallFrame.value;
            lastDivider = !!cbDivider.value;
            lastFillLeft = !!cbFillLeft.value;
            lastFillRight = !!cbFillRight.value;
            lastStrokeWidth = sw;
            lastCornerRadius = cr;
            lastBalanceMode = bm || 'none';
            lastWidthPercent = wp;
            previewFn(v, cbPreview.value, lastOverallFrame, lastDivider, lastFillLeft, lastFillRight, lastStrokeWidth, lastCornerRadius, lastBalanceMode, lastWidthPercent);
        }
        ...
        cbPreview.onClick = function () {
            if (!cbPreview.value) {
                // Preview OFF: まず確実に巻き戻す
                try { previewMgr.rollback(); } catch (e0) { }
                try { removeMarkedPreviewItems(doc); } catch (e1) { }
                previewApplied = false;
            }
            applyPreview();
        };
        ...
        okBtn.onClick = function () {
            ...
            // プレビューOFFの場合、現時点では何も適用されていないはずなので巻き戻しだけしておく
            if (!cbPreview.value) {
                try { previewMgr.rollback(); } catch (e0) { }
                clearPreviewFn();
            }
    ...
    // --- 確定処理 ---
    // OK時：Undo履歴を綺麗にするため、プレビューを一度ロールバックしてから本番を1回だけ実行
    // （Ctrl+Z 一回で戻せる状態を狙う）
    previewMgr.confirm(function () {
        var finalHr = heightRatio;
        drawBackgroundRects(item1, item2, finalHr, lastOverallFrame, lastDivider, lastFillLeft, lastFillRight, lastStrokeWidth, lastCornerRadius, lastBalanceMode, lastWidthPercent);
        // 本番生成物にはプレビューマークを付けない
        try { removeMarkedPreviewItems(doc); } catch (eMarkFinal) { }
    });
