...
- v2.7 (20260131) : ［OK］確定時にUI最新値を必ず採用（プレビューON/OFFやonChange未発火でも遡らず確定するよう修正）。
*/

// --- Version / バージョン ---
var SCRIPT_VERSION = "v2.7";
 ...
 * ========================================= */
    ...
    function showHeightDialog(defaultPercent, previewFn, clearPreviewFn) {
        ...
        // --- OK確定時にonClose側のUndoを抑止するためのフラグ ---
        var isConfirmedByOK = false;
        ...
        okBtn.onClick = function () {
            isConfirmedByOK = true;

            // UIから直接読み取って確定設定を作る（プレビューの有無やonChange未発火に依存しない）
            var p = parsePercent();
            if (p === null) {
            ...
            dlg.__finalSettings = {
                percent: p,
                addOverallFrame: !!cbOverallFrame.value,
                addDivider: !!cbDivider.value,
                addFillLeft: !!cbFillLeft.value,
                addFillRight: !!cbFillRight.value,
                strokeWidthPt: swPt,
                cornerRadiusPt: crPt,
                balanceMode: getBalanceMode(),
                widthPt: parseWidthPercent(),
        ...
        // OK時に確定用設定が組み立てられていない場合（念のため）
        if (!dlg.__finalSettings) {
            // UIから直接読み取って確定設定を作る（プレビューOFFやonChange未発火でもOK）
            var p = parsePercent();
            if (p === null) return null;

            var swPt = parseStrokeWidth();
            if (swPt === null) return null;

            var crPt = parseCornerRadius();
            if (crPt === null) return null;
    ...
    var finalSettings = showHeightDialog(DEFAULT_HEIGHT_PERCENT, previewFn, clearPreviewRects);
    if (finalSettings === null) return;

    var sizeRatio = finalSettings.percent / 100;
    ...
    var finalResult = drawBackgroundRects(
        itemA, itemB,
        sizeRatio,
        finalSettings.addOverallFrame,
        finalSettings.addDivider,
        finalSettings.addFillLeft,
        finalSettings.addFillRight,
        finalSettings.strokeWidthPt,
        finalSettings.cornerRadiusPt,
        finalSettings.balanceMode,
        finalSettings.widthPt,
        finalSettings.directionMode
    );
