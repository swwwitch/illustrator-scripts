 ...
 * ========================================= */
    ...
    function showColorPickerDialog(initColorStr) {
        ...
        previewOrig.preferredSize = [40, 40];
        var previewCur = previewRow.add('group');
        previewCur.preferredSize = [40, 40];
        ...
        // --- Tab 1: RGB / HEX ---
        var tabRGB = tabPanel.add('tab', undefined, 'RGB');
        ...
        // --- Tab 2: CMYK ---
        var tabCMYK = tabPanel.add('tab', undefined, 'CMYK');
        ...
        if (isCmyk || isGrayInit) {
            tabPanel.selection = tabCMYK;
            if (isGrayInit) cbGray.value = true;
        } else {
            tabPanel.selection = tabRGB;
        }

        // モード判定ヘルパー
        function isRGBMode() { return tabPanel.selection === tabRGB; }
        function isCMYKMode() { return tabPanel.selection === tabCMYK && !cbGray.value; }
        function isGrayMode() { return tabPanel.selection === tabCMYK && cbGray.value; }
        ...
        function updateModeVisibility() {
            ...
            tabPanel.enabled = custom;
            // グレーモード: Kのみアクティブ、C/M/Yはディム
            cRow.row.enabled = !cbGray.value;
            mRow.row.enabled = !cbGray.value;
            yRow.row.enabled = !cbGray.value;
        }
        ...
        try { _colorPickerLastPos = [d.location[0], d.location[1]]; } catch (eBeforeShow) { }
        var result = d.show();
        try { _colorPickerLastPos = [d.location[0], d.location[1]]; } catch (eAfterShow) { }
    ...
    function showHeightDialog(previewFn, clearPreviewRects) {
        ...
        function buildOptionPanel(parent) {
            var panel = parent.add('panel', undefined, L('labelCornerRadius'));
            panel.orientation = 'column';
            panel.alignChildren = ['left', 'top'];
            panel.margins = [15, 20, 15, 10];
            panel.alignment = ['fill', 'top'];
            ...
            var cbCornerLocal = row.add('checkbox', undefined, '');
        ...
        function getCurrentUIValues() {
            ...
            var cornerRadius = 0;
            if (cbCorner.value) {
                if (cbCornerAuto.value) {
                    cornerRadius = unitToPt(Number(maxCornerUnit), "rulerType");
                } else {
                    cornerRadius = unitToPt(Number(etCorner.text), "rulerType");
                }
        ...
        var optionUI = buildOptionPanel(leftColFL);
        ...
        dlg.addEventListener('keydown', function (event) {
            if (event.keyName === 'H') {
                ...
                cbCorner.value = !cbCorner.value;
                updateCornerState(); applyPreview();
                event.preventDefault();
            }
        });
    ...
    function updatePreviewLastState(addOverallFrame, addDivider, addFillLeft, addFillRight, strokeWidth, cornerRadius, widthOffset, splitDir) {
    ...
    function previewFn(enabled, addOverallFrame, addDivider, addFillLeft, addFillRight, strokeWidth, cornerRadius, widthOffset, splitDir) {
        ...
        try {
            ...
            updatePreviewLastState(addOverallFrame, addDivider, addFillLeft, addFillRight, strokeWidth, cornerRadius, widthOffset, splitDir);
    ...
    var finalResult = drawBackgroundRects(
        ...
        finalUI.strokeWidth, finalUI.cornerRadius,
