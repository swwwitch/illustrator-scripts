...
(function () {
    ...
    function normalizeRectCandidate(rectCandidate) {
        // If user selects an already-cut open path (or a non-4pt shape), reconstruct a 4-pt closed rectangle.
        // Returns a PathItem that is a closed 4-pt rectangle.
        if (!rectCandidate || rectCandidate.typename !== "PathItem") return null;
        if (isRectPathItem(rectCandidate)) return rectCandidate;

        // Build rectangle from anchor bounds
        var b = extractRectBoundsFromPathItem(rectCandidate);
        var layer = rectCandidate.layer;
        var newRect = makeClosedRectFromBounds(b, layer);

        // Copy appearance from the candidate to the new rectangle
        try {
            newRect.stroked = rectCandidate.stroked;
            newRect.filled = rectCandidate.filled;
            try { newRect.strokeColor = rectCandidate.strokeColor; } catch (_) { }
            try { newRect.fillColor = rectCandidate.fillColor; } catch (_) { }
            try { newRect.strokeWidth = rectCandidate.strokeWidth; } catch (_) { }
            try { newRect.strokeDashes = rectCandidate.strokeDashes; } catch (_) { }
            try { newRect.dashOffset = rectCandidate.dashOffset; } catch (_) { }
            try { newRect.strokeCap = rectCandidate.strokeCap; } catch (_) { }
            try { newRect.strokeJoin = rectCandidate.strokeJoin; } catch (_) { }
            try { newRect.miterLimit = rectCandidate.miterLimit; } catch (_) { }
            try { newRect.strokeOverprint = rectCandidate.strokeOverprint; } catch (_) { }
            try { newRect.opacity = rectCandidate.opacity; } catch (_) { }
            try { newRect.blendingMode = rectCandidate.blendingMode; } catch (_) { }
        } catch (e) { }
        ...
        // Remove old cut path / non-rect candidate
    ...
    // Accept either a strict rectangle, or an already-cut path (open) / non-4pt path and normalize it.
    if (isTextItem(a) && isPathItem(b)) { tf = a; rectA = b; }
    else if (isTextItem(b) && isPathItem(a)) { tf = b; rectA = a; }
    ...
    function applyPreview(marginMM) {
        ...
        // Decide which edge to cut based on the text position
        buildTextBoundsForCalcOnce();
        var tbEdge = __textBoundsForCalc || tf.visibleBounds;
        var cutEdge = chooseCutEdge(tbEdge, rL, rT, rR, rBot); // "top" | "bottom" | "left" | "right"
        ...
        if (cutEdge === "bottom") {
            ...
            var tb = __textBoundsForCalc || tf.visibleBounds;
            var cutT = tb[1] + addPt;
            var cutB = tb[3] - addPt;
            if (cutT > rT) cutT = rT;
            ...
            var tb2 = __textBoundsForCalc || tf.visibleBounds;
            var cutT2 = tb2[1] + addPt;
            var cutB2 = tb2[3] - addPt;
            if (cutT2 > rT) cutT2 = rT;
            if (cutB2 < rBot) cutB2 = rBot;
