...
function main() {
    ...
    var previewItems = [];
    function groupPreviewLeaderParts(container, whitePath, newPath, whiteCircle, circle, hasWhiteEdge) {
        if (hasWhiteEdge) {
            var edgeGrp = container.groupItems.add();
            var mainGrp = container.groupItems.add();
    ...
    // 白フチON時は、本体とフチを別サブグループに分けて構造を固定する関数
    // 親グループ
    //   ├─ 本体サブグループ   : 線 → 線端の丸
    //   └─ フチサブグループ   : フチの線 → 線端の丸
    function groupLeaderParts(container, whitePath, newPath, whiteCircle, circle, hasWhiteEdge) {
        if (hasWhiteEdge) {
            var edgeGrp = container.groupItems.add();
            var mainGrp = container.groupItems.add();

            try { mainGrp.note = "leader_line_main"; } catch (e) { }
            try { edgeGrp.note = "leader_line_edge"; } catch (e) { }

            if (newPath) {
                try { newPath.note = "leader_line_main_path"; } catch (e) { }
                newPath.move(mainGrp, ElementPlacement.PLACEATEND);
            }
            if (circle) {
                try { circle.note = "leader_line_main_cap"; } catch (e) { }
                circle.move(mainGrp, ElementPlacement.PLACEATEND);
            }

            if (whitePath) {
                try { whitePath.note = "leader_line_edge_path"; } catch (e) { }
                whitePath.move(edgeGrp, ElementPlacement.PLACEATEND);
            }
            if (whiteCircle) {
                try { whiteCircle.note = "leader_line_edge_cap"; } catch (e) { }
                whiteCircle.move(edgeGrp, ElementPlacement.PLACEATEND);
            }
        } else {
            if (newPath) {
                try { newPath.note = "leader_line_main_path"; } catch (e) { }
                newPath.move(container, ElementPlacement.PLACEATEND);
            }
            if (circle) {
                try { circle.note = "leader_line_main_cap"; } catch (e) { }
                circle.move(container, ElementPlacement.PLACEATEND);
            }
        }
    }
    ...
    function createPreview(angleDeg) {
        ...
        for (var i = 0; i < boundsData.length; i++) {
            ...
            groupPreviewLeaderParts(previewGrp, whitePath, newPath, whiteCircle, circle, whiteEdgeCheck.value);
    ...
    for (var i = 0; i < targets.length; i++) {
        ...
        if (groupCheck.value) {
            ...
            groupLeaderParts(grp, whitePath, newPath, whiteCircle, circle, whiteEdgeCheck.value);
            setLeaderLineTag(grp);
            setLeaderLineBoundsTags(grp, b);
            grp.selected = true;
        } else {
            if (newPath) newPath.selected = true;
            if (circle) circle.selected = true;
            if (whitePath) whitePath.selected = true;
            if (whiteCircle) whiteCircle.selected = true;
        }

        // leader_line 再適用時も、参照は A線のみだが置き換えは旧グループ全体に対して行う
        // 元のパスを削除
        targets[i].remove();
    }
}
