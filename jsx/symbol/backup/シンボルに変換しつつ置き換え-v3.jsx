...
- 選択 1 個を複製し、指定したシンボル名と基準点でシンボル登録する
...
function toSymbolRegistrationPoint(referencePointIndex) {
  return SYMBOL_REGISTRATION_POINT_BY_INDEX[referencePointIndex] || SymbolRegistrationPoint.SYMBOLCENTERPOINT;
}
...
/* 同じフォント・スタイル・サイズかつ同一文字列のテキストフレームを取得 / Collect TextFrames matching font/style/size and exact text content */
...
function replaceItemsWithSymbol(targetDocument, targetItems, destinationSymbol, referencePoint, isGlobalEdit) {
  ...
  if (skippedCount > 0) {
    alert(skippedCount + ' 件はロック／非表示、または親レイヤーの状態により置換できませんでした。\n' +
      skippedCount + ' item(s) could not be replaced because they or their parent layers were locked or hidden.');
  }
}
