...
(function () {
    ...
    */
    ...
    /* 罫線Y範囲取得（線幅を無視＝中心線基準）/ Compute Y bounds of rule lines (centerline-based, ignoring stroke width) */
    function computeRuleLineYBounds(doc, excludeLayerNames) {
        ...
        for (var layerIndex = 0; layerIndex < doc.layers.length; layerIndex++) {
            ...
            if (shouldSkipLayer || layer.locked || !layer.visible) continue;
            var childBounds = collectRuleLineYBoundsInItems(layer.pageItems);
            if (childBounds) {
                if (maxY === null || childBounds.maxY > maxY) maxY = childBounds.maxY;
                if (minY === null || childBounds.minY < minY) minY = childBounds.minY;
            }
        }
        if (maxY === null || minY === null) return null;
        return { topY: maxY, bottomY: minY };
    }

    /* Y範囲再帰収集（水平罫線の中心Yのみ採用）/ Recursively collect Y bounds (horizontal rule centerlines only) */
    function collectRuleLineYBoundsInItems(items) {
        var maxY = null, minY = null;
        for (var itemIndex = 0; itemIndex < items.length; itemIndex++) {
            ...
            try {
                ...
                if (pageItem.typename === "PathItem") {
                    if (pageItem.pathPoints && pageItem.pathPoints.length === 2) {
                        var startAnchor = pageItem.pathPoints[0].anchor;
                        var endAnchor = pageItem.pathPoints[1].anchor;
                        if (Math.abs(startAnchor[0] - endAnchor[0]) >= Math.abs(startAnchor[1] - endAnchor[1])) {
                            var lineY = (startAnchor[1] + endAnchor[1]) / 2;
                            if (maxY === null || lineY > maxY) maxY = lineY;
                            if (minY === null || lineY < minY) minY = lineY;
                        }
                    } else if (pageItem.closed && pageItem.pathPoints && pageItem.pathPoints.length === 4) {
                        var rectBounds = pageItem.geometricBounds;
                        var rectWidth = rectBounds[2] - rectBounds[0];
                        var rectHeight = rectBounds[1] - rectBounds[3];
                    ...
                    var childBounds = collectRuleLineYBoundsInItems(pageItem.pageItems);
                    if (childBounds) {
                        if (maxY === null || childBounds.maxY > maxY) maxY = childBounds.maxY;
                        if (minY === null || childBounds.minY < minY) minY = childBounds.minY;
                    }
    ...
    /* 罫線X範囲取得（線幅を無視＝中心線基準）/ Compute X bounds of rule lines (centerline-based) */
    function computeRuleLineXBounds(doc, excludeLayerNames) {
        ...
        for (var layerIndex = 0; layerIndex < doc.layers.length; layerIndex++) {
            ...
            if (shouldSkipLayer || layer.locked || !layer.visible) continue;
            var childBounds = collectRuleLineXBoundsInItems(layer.pageItems);
            if (childBounds) {
                if (maxX === null || childBounds.maxX > maxX) maxX = childBounds.maxX;
                if (minX === null || childBounds.minX < minX) minX = childBounds.minX;
            }
        }
        if (maxX === null || minX === null) return null;
        return { leftX: minX, rightX: maxX };
    }

    /* X範囲再帰収集（垂直罫線の中心Xのみ採用）/ Recursively collect X bounds (vertical rule centerlines only) */
    function collectRuleLineXBoundsInItems(items) {
        var maxX = null, minX = null;
        for (var itemIndex = 0; itemIndex < items.length; itemIndex++) {
            var pageItem = items[itemIndex];
            try {
                ...
                if (pageItem.typename === "PathItem") {
                    if (pageItem.pathPoints && pageItem.pathPoints.length === 2) {
                        var startAnchor = pageItem.pathPoints[0].anchor;
                        var endAnchor = pageItem.pathPoints[1].anchor;
                        if (Math.abs(startAnchor[1] - endAnchor[1]) > Math.abs(startAnchor[0] - endAnchor[0])) {
                            var lineX = (startAnchor[0] + endAnchor[0]) / 2;
                            if (maxX === null || lineX > maxX) maxX = lineX;
                            if (minX === null || lineX < minX) minX = lineX;
                        }
                    } else if (pageItem.closed && pageItem.pathPoints && pageItem.pathPoints.length === 4) {
                        var rectBounds = pageItem.geometricBounds;
                        var rectWidth = rectBounds[2] - rectBounds[0];
                        ...
                        if (rectHeight > rectWidth) {
                            ...
                            if (maxX === null || centerX > maxX) maxX = centerX;
                            if (minX === null || centerX < minX) minX = centerX;
                        }
                    }
                } else if (pageItem.typename === "GroupItem") {
                    var childBounds = collectRuleLineXBoundsInItems(pageItem.pageItems);
                    if (childBounds) {
                        if (maxX === null || childBounds.maxX > maxX) maxX = childBounds.maxX;
                        if (minX === null || childBounds.minX < minX) minX = childBounds.minX;
                    }
        ...
        if (maxX === null || minX === null) return null;
        return { maxX: maxX, minX: minX };
    }

    /* 矩形のストローク幅を取得 / Get stroke width (0 if not stroked) */
    function getRectStrokeWidth(rect) {
        try {
            if (!rect.stroked) return 0;
            var w = rect.strokeWidth;
            return (typeof w === "number" && w > 0) ? w : 0;
        } catch (e) { return 0; }
    }

    /* ジオメトリ境界で矩形を配置／リサイズ（パス座標を直接変更）
       Place/resize rectangle by directly setting path anchors */
    function setRectGeometricFrame(rect, geoLeft, geoTop, geoWidth, geoHeight) {
        setRectanglePathBounds(rect, geoLeft, geoTop, geoLeft + geoWidth, geoTop - geoHeight);
    }

    /* 矩形パスのアンカーを境界に合わせる / Set rectangle path anchors to exact bounds */
    function setRectanglePathBounds(rect, left, top, right, bottom) {
        if (!rect || !rect.pathPoints || rect.pathPoints.length !== 4) return;
        ...
        safeCall(function () {
            for (var pointIndex = 0; pointIndex < 4; pointIndex++) {
                var currentAnchor = rect.pathPoints[pointIndex].anchor;
                var newPoint;
                if (currentAnchor[0] <= centerX && currentAnchor[1] >= centerY) {
                    newPoint = [left, top];
                } else if (currentAnchor[0] > centerX && currentAnchor[1] >= centerY) {
                    newPoint = [right, top];
                } else if (currentAnchor[0] > centerX && currentAnchor[1] < centerY) {
                ...
                rect.pathPoints[pointIndex].anchor = newPoint;
                rect.pathPoints[pointIndex].leftDirection = newPoint;
                rect.pathPoints[pointIndex].rightDirection = newPoint;
            }
        });
    }

    /* セル背景の4辺を直近の罫線中心へスナップ / Snap cell background edges to nearest rule centerlines */
    function snapCellBackgroundsToRuleLines(doc, layerName, ruleItems, insetPt) {
        var ruleCenters = collectRuleCenterPositionsFromItems(ruleItems);
        if (!ruleCenters || ruleCenters.xPositions.length < 2 || ruleCenters.yPositions.length < 2) return;

        var rectangleItems = collectCellBackgroundSnapTargetsInLayer(doc, layerName);
        for (var rectangleIndex = 0; rectangleIndex < rectangleItems.length; rectangleIndex++) {
            try {
    ...
    /* セル背景スナップ対象を収集 / Collect cell background targets for snapping */
    function collectCellBackgroundSnapTargetsInLayer(doc, layerName) {
        var result = [];
        for (var layerIndex = 0; layerIndex < doc.layers.length; layerIndex++) {
            var layer = doc.layers[layerIndex];
            if (layer.name !== layerName) continue;
            if (layer.locked || !layer.visible) return result;
            collectCellBackgroundSnapTargetsInItems(layer.pageItems, result);
            break;
        }
        return result;
    }
    ...
    /* セル背景を罫線幅の半分だけ内側に縮める / Inset cell backgrounds by half of rule stroke width */
    function insetCellBackgroundsByStrokeWidth(doc, layerName, strokeWidthPt) {
        // strokeWidthPtに依存せず常に実行
        // 固定で0.5pt内側に縮める / Always inset by 0.5pt
        var inset = 0.5;
        var rectangleItems = collectRectanglesInLayer(doc, layerName);
        for (var rectangleIndex = 0; rectangleIndex < rectangleItems.length; rectangleIndex++) {
            try {
                var rectangleItem = rectangleItems[rectangleIndex];
                var bounds = rectangleItem.geometricBounds;
                var geoLeft = bounds[0];
                var geoTop = bounds[1];
                var geoWidth = bounds[2] - bounds[0];
                var geoHeight = bounds[1] - bounds[3];
                setInsetRectGeometricFrame(rectangleItem, geoLeft, geoTop, geoWidth, geoHeight, inset);
            } catch (e) { }
        }
    }

    /* ジオメトリ境界を内側に縮めて矩形を配置／リサイズ
       Place/resize rectangle by inset geometric bounds */
    function setInsetRectGeometricFrame(rect, geoLeft, geoTop, geoWidth, geoHeight, inset) {
        var safeWidth = Math.max(geoWidth - inset * 2, 0.01);
        var safeHeight = Math.max(geoHeight - inset * 2, 0.01);
        setRectanglePathBounds(
            rect,
            geoLeft + inset,
            geoTop - inset,
            geoLeft + inset + safeWidth,
            geoTop - inset - safeHeight
        );
    }

    /* セル高さ再配置 / Adjust cell heights by bounds */
    function adjustCellsByYBounds(doc, layerName, topY, bottomY) {
        var rectangleItems = collectRectanglesInLayer(doc, layerName);
        if (rectangleItems.length < 1 || topY <= bottomY) return;

        var totalSpan = topY - bottomY;
        var tolerance = 0.5;
        var columns = [];

        for (var rectangleIndex = 0; rectangleIndex < rectangleItems.length; rectangleIndex++) {
            var rectangleItem = rectangleItems[rectangleIndex];
            var columnLeftPosition;
            try { columnLeftPosition = rectangleItem.geometricBounds[0]; } catch (e) { continue; }
            var placed = false;
            for (var columnIndex = 0; columnIndex < columns.length; columnIndex++) {
                if (Math.abs(columns[columnIndex].columnLeft - columnLeftPosition) < tolerance) {
                    columns[columnIndex].items.push(rectangleItem);
                    placed = true;
                    break;
                }
            }
            if (!placed) {
                columns.push({ columnLeft: columnLeftPosition, items: [rectangleItem] });
            }
        }

        for (var stackIndex = 0; stackIndex < columns.length; stackIndex++) {
            var column = columns[stackIndex];
            var rectangleCount = column.items.length;
            if (rectangleCount === 0) continue;
            column.items.sort(function (rectangleA, rectangleB) {
                try { return rectangleB.geometricBounds[1] - rectangleA.geometricBounds[1]; } catch (e) { return 0; }
            });
            var newHeight = totalSpan / rectangleCount;
            for (var itemIndex = 0; itemIndex < rectangleCount; itemIndex++) {
                try {
                    var rectangleItemInColumn = column.items[itemIndex];
                    var bounds = rectangleItemInColumn.geometricBounds;
                    var currentGeoLeft = bounds[0];
                    var currentGeoWidth = bounds[2] - bounds[0];
                    var newGeoTop = topY - newHeight * itemIndex;
                    setRectGeometricFrame(rectangleItemInColumn, currentGeoLeft, newGeoTop, currentGeoWidth, newHeight);
                } catch (e) { }
            }
        }
    }

    /* セル幅再配置 / Adjust cell widths by bounds */
    function adjustCellsByXBounds(doc, layerName, leftX, rightX) {
        var rectangleItems = collectRectanglesInLayer(doc, layerName);
        if (rectangleItems.length < 1 || leftX >= rightX) return;

        var totalSpan = rightX - leftX;
        var tolerance = 0.5;
        var rows = [];
        ...
        for (var rectangleIndex = 0; rectangleIndex < rectangleItems.length; rectangleIndex++) {
            var rectangleItem = rectangleItems[rectangleIndex];
            var rowTopPosition;
            try { rowTopPosition = rectangleItem.geometricBounds[1]; } catch (e) { continue; }
            var placed = false;
            for (var rowIndex = 0; rowIndex < rows.length; rowIndex++) {
                if (Math.abs(rows[rowIndex].rowTop - rowTopPosition) < tolerance) {
                    rows[rowIndex].items.push(rectangleItem);
                    placed = true;
                    break;
                }
            }
            if (!placed) {
                rows.push({ rowTop: rowTopPosition, items: [rectangleItem] });
            }
        }

        for (var rIdx = 0; rIdx < rows.length; rIdx++) {
            var row = rows[rIdx];
            var cellCount = row.items.length;
            if (cellCount === 0) continue;
            row.items.sort(function (cellA, cellB) {
                try { return cellA.geometricBounds[0] - cellB.geometricBounds[0]; } catch (e) { return 0; }
            });
            var newWidth = totalSpan / cellCount;
            for (var iIdx = 0; iIdx < cellCount; iIdx++) {
                try {
                    var cell = row.items[iIdx];
                    var cellBounds = cell.geometricBounds;
                    var currentGeoTop = cellBounds[1];
                    var currentGeoHeight = cellBounds[1] - cellBounds[3];
                    var newGeoLeft = leftX + newWidth * iIdx;
                    setRectGeometricFrame(cell, newGeoLeft, currentGeoTop, newWidth, currentGeoHeight);
                } catch (e) { }
            }
        }
    }
    ...
    /* レイヤー内矩形収集 / Collect rectangles in layer */
    ...
    /* 再帰矩形走査 / Recursively walk rectangles */
    ...
    /* 高さ均等化 / Equalize rectangle heights */
    function equalizeRectangleHeightsInLayer(doc, layerName) {
        var rectangleItems = collectRectanglesInLayer(doc, layerName);
        if (rectangleItems.length < 1) return;

        var minX = null, maxX = null;
        for (var rectangleIndex = 0; rectangleIndex < rectangleItems.length; rectangleIndex++) {
            try {
                var geometricBounds = rectangleItems[rectangleIndex].geometricBounds;
                if (minX === null || geometricBounds[0] < minX) minX = geometricBounds[0];
                if (maxX === null || geometricBounds[2] > maxX) maxX = geometricBounds[2];
            } catch (e) { }
        }
        if (minX === null || maxX === null) return;

        var newHeight = (maxX - minX) / rectangleItems.length;
        if (newHeight <= 0) return;

        for (var itemIndex = 0; itemIndex < rectangleItems.length; itemIndex++) {
            try {
                var rectangleItem = rectangleItems[itemIndex];
                var bounds = rectangleItem.geometricBounds;
                var geoHeight = bounds[1] - bounds[3];
                if (Math.abs(geoHeight - newHeight) < 0.01) continue;
                var geoLeft = bounds[0];
                var geoTop = bounds[1];
                var geoWidth = bounds[2] - bounds[0];
                setRectGeometricFrame(rectangleItem, geoLeft, geoTop, geoWidth, newHeight);
            } catch (e) { }
        }

        stackRectanglesVertically(rectangleItems);
    }

    /* 縦積み配置 / Stack rectangles vertically */
    function stackRectanglesVertically(rectangleItems) {
        if (rectangleItems.length < 2) return;
        ...
        for (var stackIndex = 0; stackIndex < columns.length; stackIndex++) {
            var column = columns[stackIndex];
            if (column.items.length < 2) continue;
            column.items.sort(function (rectangleA, rectangleB) {
                try { return rectangleB.geometricBounds[1] - rectangleA.geometricBounds[1]; } catch (e) { return 0; }
            });
            for (var itemIndex = 1; itemIndex < column.items.length; itemIndex++) {
                try {
                    var previousRectangle = column.items[itemIndex - 1];
                    var currentRectangle = column.items[itemIndex];
                    var prevBounds = previousRectangle.geometricBounds;
                    var curBounds = currentRectangle.geometricBounds;
                    var nextGeoTop = prevBounds[3];
                    var curGeoLeft = curBounds[0];
                    var curGeoWidth = curBounds[2] - curBounds[0];
                    var curGeoHeight = curBounds[1] - curBounds[3];
                    setRectGeometricFrame(currentRectangle, curGeoLeft, nextGeoTop, curGeoWidth, curGeoHeight);
                } catch (e) { }
            }
        }
    }
