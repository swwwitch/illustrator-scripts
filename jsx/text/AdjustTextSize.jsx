...
function main() {
    ...
    var rbOneTime = modePanel.add("radiobutton", undefined, "ワンタイム");
    var rbStack = modePanel.add("radiobutton", undefined, "重ねて指定");
    rbOneTime.value = true;
    ...
    var targetPanel = leftCol.add("panel", undefined, LABELS.targetChar[lang]);
    ...
    var radioRow1 = targetRadioGroup.add("group");
    radioRow1.orientation = "row";
    var rbHiragana = radioRow1.add("radiobutton", undefined, "ひらがな");
    var rbJoshi = radioRow1.add("radiobutton", undefined, "助詞");

    var radioRow2 = targetRadioGroup.add("group");
    radioRow2.orientation = "row";
    var rbDate = radioRow2.add("radiobutton", undefined, "日付（年月日）");
    var rbYenEtc = radioRow2.add("radiobutton", undefined, "¥、円、％");

    var radioRow3 = targetRadioGroup.add("group");
    radioRow3.orientation = "row";
    var rbHyphen = radioRow3.add("radiobutton", undefined, "-（ハイフン）");
    var rbComma = radioRow3.add("radiobutton", undefined, ",（カンマ）");

    var radioRow4 = targetRadioGroup.add("group");
    radioRow4.orientation = "row";
    var rbNumbers = radioRow4.add("radiobutton", undefined, "数字");
    var rbAlpha = radioRow4.add("radiobutton", undefined, "アルファベット");

    var radioRow5 = targetRadioGroup.add("group");
    radioRow5.orientation = "row";
    var rbAlphaNum = radioRow5.add("radiobutton", undefined, "アルファベット＋数字");

    var rbSpecify = targetRadioGroup.add("radiobutton", undefined, "指定");

    // ラジオボタンの排他制御（グループが異なるため手動で管理）
    var allRadios = [rbHiragana, rbJoshi, rbDate, rbYenEtc, rbHyphen, rbComma, rbNumbers, rbAlpha, rbAlphaNum, rbSpecify];
    function deselectOtherRadios(selected) {
        for (var i = 0; i < allRadios.length; i++) {
            if (allRadios[i] !== selected) {
                allRadios[i].value = false;
            }
        }
    }

    var targetTextGroup = targetPanel.add("group");
    targetTextGroup.orientation = "row";
    var targetCharInput = targetTextGroup.add("edittext", undefined, uniqueNonAN);
    targetCharInput.characters = 24;

    rbSpecify.value = true;
    ...
    function commitAndReset() {
        if (rbOneTime.value) {
            ...
            // 重ねて指定: 現在のプレビューを確定し、undoDepthをリセット
            previewMgr.undoDepth = 0;
        }
    }
    ...
    rbSpecify.onClick = function () {
        deselectOtherRadios(rbSpecify);
        commitAndReset();
        }
        targetCharInput.enabled = true;
        targetCharInput.notify("onChange");
    };
    rbHiragana.onClick = function () {
        deselectOtherRadios(rbHiragana);
        setTargetCharAndDim(getUniqueHiragana(allSelText));
    };
    rbJoshi.onClick = function () {
        deselectOtherRadios(rbJoshi);
        setTargetCharAndDim("やでよりからとへにをがのは");
    };
    rbDate.onClick = function () {
        deselectOtherRadios(rbDate);
        setTargetCharAndDim("0123456789");
    };
    rbYenEtc.onClick = function () {
        deselectOtherRadios(rbYenEtc);
        setTargetCharAndDim("¥￥円％%");
    };
    rbHyphen.onClick = function () {
        deselectOtherRadios(rbHyphen);
        setTargetCharAndDim("-");
    };
    rbComma.onClick = function () {
        deselectOtherRadios(rbComma);
        setTargetCharAndDim(",");
    };
    rbNumbers.onClick = function () {
        deselectOtherRadios(rbNumbers);
        setTargetCharAndDim("0123456789");
    };
    rbAlpha.onClick = function () {
        deselectOtherRadios(rbAlpha);
        setTargetCharAndDim("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz");
    };
    rbAlphaNum.onClick = function () {
        deselectOtherRadios(rbAlphaNum);
        setTargetCharAndDim("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789");
    };
