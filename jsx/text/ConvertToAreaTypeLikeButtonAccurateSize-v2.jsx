...
(function () {
    ...
    /* ［形状に変換：長方形］ライブエフェクトの定義（追加モード：幅・高さに追加）
       "Convert to Shape: Rectangle" live-effect definition (Add mode: extra width / extra height) */
    function buildRectangleShapeEffectXML(addWidth, addHeight) {
        var w = Math.round(addWidth * 1000) / 1000;
        var h = Math.round(addHeight * 1000) / 1000;
        return '<LiveEffect name="Adobe Shape Effects" isPre="1"><Dict data="U DisplayString Rectangle I Shape 0 R RelWidth ' + w + ' R RelHeight ' + h + ' R AbsWidth 0 R AbsHeight 0 R Absolute 0 R CornerRadius 9 "/></LiveEffect>';
    }

    /* 塗りを2つ追加し、長方形のシェイプ効果でボタン状の背景にする（幅・高さに追加を指定）
       Add two fills and a rectangle shape effect for a button-like background (with extra width / height) */
    function applyButtonShapeAppearance(item, addWidth, addHeight) {
        try {
            ...
            app.executeMenuCommand('Adobe New Fill Shortcut');
            item.applyEffect(buildRectangleShapeEffectXML(addWidth, addHeight));
        } catch (e) { }
    }
    ...
    function convertSelectionToAreaType(doc, sel) {
        ...
        // 計測した実寸でフレームを作り、エリア内文字へ変換 / Frame at the measured real size, converted to area type
        var created = convertPointTextToMeasuredArea(doc, selection);
    ...
    /* エリア内文字 → ポイント文字（逆変換）。元のフレーム実寸に合わせて長方形シェイプ効果を付与
       Area type → point text (reverse); add a rectangle shape effect matching the original frame size */
    function convertAreaTypeToPointText(doc, selection) {
        var created = [];

        for (var i = selection.length - 1; i >= 0; i--) {
            var areaText = selection[i];
            if (!(areaText.typename === "TextFrame" && areaText.kind === TextType.AREATEXT)) continue;

            // 1) エリア文字の幅・高さ（A, B）/ Area text width & height (A, B)
            var frameBounds = geometricBoundsOf(areaText);
            var frameWidth = frameBounds.width;   // A
            var frameHeight = frameBounds.height; // B
            if (frameWidth <= 0 || frameHeight <= 0) continue;

            // 2) ポイント文字へ変換（C）/ Convert to point text (C)
            var pointText = null;
            try { pointText = areaText.convertAreaObjectToPointObject(); } catch (e) { }
            if (!pointText) continue;

            // 3) 複製→アウトライン化で文字の実寸（D, E）を計測（複製は破棄）
            //    Measure the text's real size (D, E) via duplicate → outline (the copy is discarded)
            var textBounds = measureAccurateBounds(pointText) || geometricBoundsOf(pointText);
            var textWidth = textBounds.width;   // D
            var textHeight = textBounds.height; // E

            // 4) ［形状に変換：長方形］効果を付与。幅に追加 = A - D、高さに追加 = B - E
            //    Apply Convert-to-Shape (Rectangle); extra width = A - D, extra height = B - E
            var addWidth = frameWidth - textWidth;
            var addHeight = frameHeight - textHeight;
            if (addWidth < 0) addWidth = 0;
            if (addHeight < 0) addHeight = 0;
            applyButtonShapeAppearance(pointText, addWidth, addHeight);

            created.push(pointText);
        }

        if (created.length > 0) {
            try { doc.selection = created; app.redraw(); } catch (e) { }
        }
    ...
    if (app.documents.length > 0) {
        ...
        if (sel && sel.length > 0) {
            // 選択内容から変換方向を判定 / Detect conversion direction from the selection
            ...
            for (var i = 0; i < sel.length; i++) {
                ...
                else if (sel[i].kind === TextType.POINTTEXT || sel[i].kind === TextType.PATHTEXT) hasConvertibleText = true;
            }

            if (hasAreaText) {
                // エリア内文字 → ポイント文字（逆変換）/ Area type → point text (reverse)
                convertAreaTypeToPointText(doc, sel);
            } else if (hasConvertibleText) {
                // ポイント文字・パス上文字 → エリア内文字。フレーム整列アクションを読み込み、終了時に破棄
                // Point / path text → area type. Load frame-alignment actions, unload on exit
                loadAreaTextActions();
                try {
