...
function main() {
    ...
    // 自動調整パネル (Auto Adjust panel)
    var autoAdjustPanel = leftPanel.add("panel", undefined, "自動調整");
    autoAdjustPanel.orientation = "column";
    autoAdjustPanel.alignChildren = "left";
    autoAdjustPanel.margins = [15, 20, 15, 10];

    // 基準文字入力行を追加
    var baseCharGroup = autoAdjustPanel.add("group");
    baseCharGroup.orientation = "row";
    var baseCharLabel = baseCharGroup.add("statictext", undefined, "基準文字:");
    baseCharLabel.preferredSize.width = 60;
    var baseCharInput = baseCharGroup.add("edittext", undefined, "0");
    baseCharInput.characters = 3;

    // 「自動調整」ボタンを追加
    var autoAdjustBtn = autoAdjustPanel.add("button", undefined, "自動調整");

    // 自動調整ボタンのクリックハンドラ
    autoAdjustBtn.onClick = function() {
        if (textFrames.length !== 1) {
            alert("1つのテキストフレームを選択してください。");
            return;
        }
        var tf = textFrames[0];
        var chars = tf.textRange.characters;
        var baseChar = baseCharInput.text;
        var targetText = targetInput.text;

        if (!baseChar || baseChar.length !== 1) {
            alert("基準文字を1文字入力してください。");
            return;
        }
        var basePos = tf.contents.indexOf(baseChar);
        if (basePos === -1) {
            alert("基準文字が見つかりません。");
            return;
        }
        var baseAttr = chars[basePos].characterAttributes;
        var baseSize = baseAttr.size;
        var baseAscent = baseSize * 0.7;
        var baseDescent = baseSize * 0.3;
        var baseCenter = (baseAscent - baseDescent) / 2;

        resetBaselineShift([tf]);
        ...
        for (var c = 0; c < targetText.length; c++) {
            ...
            var pos = contents.indexOf(ch);
            while (pos !== -1) {
                var attr = chars[pos].characterAttributes;
                var ascent = attr.size * 0.7;
                var descent = attr.size * 0.3;
                var center = (ascent - descent) / 2;
                var shiftAmount = baseCenter - center;
                chars[pos].characterAttributes.baselineShift = shiftAmount;
                pos = contents.indexOf(ch, pos + 1);
            }
        }
        app.redraw();
