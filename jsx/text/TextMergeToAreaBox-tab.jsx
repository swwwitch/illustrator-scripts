...
function main() {
    ...
    if (textLines.length != 0) {
        ...
        // 最初のテキストの先頭が改行なら「〓」に置換（複数の改行にも対応）
        if (textLines.length > 0 && textLines[0].length > 0) {
            var firstItem = textLines[0][0];
            firstItem.contents = firstItem.contents.replace(/\r/g, "〓");
        }
        ...
        if (textLines.length === 1) {
            ...
            for (var j = 0; j < sortedItems.length; j++) {
                var itemContent = sortedItems[j].contents;
                if (itemContent.length > 0) {
                    itemContent = itemContent.replace(/\r/g, "〓");
                }
                mergedText += itemContent;
                if (j < sortedItems.length - 1) {
                    mergedText += "\t"; // Add tab between horizontal text items
                }
            }
        ...
        */
        ...
        for (var i = 0; i < mergedTextFrames.length; i++) {
            ...
            if (i < mergedTextFrames.length - 1) {
                // Keep vertical concatenation as line breaks
                finalText += "\r";
            }
