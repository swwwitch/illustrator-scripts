...
var SCRIPT_README_JA   = "https://github.com/swwwitch/illustrator-scripts/blob/master/readme-ja/TextMergeToAreaBox.md"; /* README（日本語） */
var SCRIPT_README_EN   = "https://github.com/swwwitch/illustrator-scripts/blob/master/readme-en/TextMergeToAreaBox.md"; /* README (English) */
...
(function () {
     ...
     * @param {string} labelPath - "alert.noText" のようなドット区切りのキー
    ...
    // =========================================
    // 行の組み立て / Line assembly
    // =========================================

    /**
     * テキストフレームをまとめて削除する
     * @param {TextFrame[]} framesToRemove - 削除するテキストフレーム
     * @returns {void}
     */
    ...
    /**
     * テキストフレームを左から右へ並び替える
     * @param {TextFrame[]} framesToSort - 対象テキストフレーム
     * @returns {TextFrame[]} 並び替えた新しい配列
     */
     ...
     * @param {TextFrame[]} framesTopToBottom - 上から下へ並び替え済みのテキストフレーム
     * @param {number} yThreshold - 同じ行と見なすY座標差の閾値（pt）
     * @returns {TextFrame[][]} 行ごとのテキストフレーム配列
     */
     ...
     * @returns {TextFrame[][]} 上から順に並べた行ごとのテキストフレーム配列
     */
     ...
     * @param {TextFrame[]} lineFrames - 同じ行に属するテキストフレーム
     ...
     * @param {TextFrame[]} mergedLineFrames - 行ごとに連結済みのテキストフレーム
     ...
     * @param {TextFrame[]} sourceFrames - 対象テキストフレーム（1つ以上）
     * @returns {number[]} [左, 上, 右, 下] の座標（pt）
     */
     ...
     * @param {TextFrame[]} mergedLineFrames - 行ごとに連結済みのテキストフレーム（2行以上）
