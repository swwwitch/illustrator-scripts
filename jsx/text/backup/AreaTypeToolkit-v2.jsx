...
if (app.documents.length > 0) {
    ...
    if (sel.length > 0) {
        ...
        function runProcess(forPreview) {
            ...
            // テキストの配置（フレーム整列）：上=0 / 中央=1 / 下=2 / 均等配置=3
            var alignValueInt = 0;
            if (radAlignCenter.value) {
                alignValueInt = 1;
            } else if (radAlignBottom.value) {
                alignValueInt = 2;
            } else if (radAlignJustify.value) {
                alignValueInt = 3;
            }
        ...
        // テキストの配置ラジオボタン：手動排他制御 + プレビュー更新
        for (var ar = 0; ar < allAlignRadios.length; ar++) {
            (function (btn) {
                btn.onClick = function () {
                    for (var j = 0; j < allAlignRadios.length; j++) {
                        allAlignRadios[j].value = (allAlignRadios[j] === btn);
                    }
                    updatePreview();
                };
            })(allAlignRadios[ar]);
