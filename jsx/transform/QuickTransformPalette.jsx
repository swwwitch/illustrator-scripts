...
/* 複製アイコン（重なる2つの四角。奥＝右上・手前＝左下）を白抜きで描画。bgColor で手前の四角下の奥線を消す */
/* Draw a duplicate icon (two overlapping squares: back = upper-right, front = lower-left), outlined; bgColor erases the back lines under the front square */
function drawDuplicateGlyph(graphics, w, h, color, bgColor) {
	var squareSize = Math.min(w, h) * 0.40;
	var shift = squareSize * 0.36;              /* 2枚のずらし量 / Offset between the two squares */
	var pairSize = squareSize + shift;
	var left = Math.round((w - pairSize) / 2);
	var top = Math.round((h - pairSize) / 2);
	...
	squarePath(backX, backY, squareSize);
	...
	if (bgColor) {
		squarePath(frontX, frontY, squareSize);
...
/* 基準点セルの□を1つ描画（全セル 塗り＋枠。通常は塗り透過・枠は薄いグレー／選択時は塗り濃いグレー・枠は薄いグレー）*/
/* Draw one anchor-cell square (every cell = fill + border; normal: transparent fill + light-gray border / selected: dark-gray fill + light-gray border) */
function drawAnchorCell(graphics, x, y, size, selected) {
	/* 選択中のみ濃いグレーで塗る（通常は透過＝塗らずパネル地色を見せる）。枠を上に描くので先に塗る / Fill dark gray only when selected (transparent otherwise); fill first so the border draws on top */
	if (selected) {
		graphics.newPath();
		graphics.moveTo(x, y);
		graphics.lineTo(x + size, y);
	...
	/* 枠線は常に薄いグレー（塗りの上に描くので選択時も枠が残る）/ Border is always light gray (drawn over the fill so it remains visible when selected) */
	graphics.newPath();
	graphics.moveTo(x, y);
	graphics.lineTo(x + size, y);
	graphics.lineTo(x + size, y + size);
...
function drawRotateIcon(graphics, width, height, mirror) {
	...
	strokeArc(graphics, color, centerX, centerY, radius, headDeg, 410, mirrorSign, ICON_LINE_WIDTH, ICON_LINE_WIDTH);
	/* 下側は四角い点線 / Square-dotted arc on the lower side */
	drawDottedArc(graphics, color, centerX, centerY, radius, 50, 150, mirrorSign, ICON_LINE_WIDTH);
...
function buildMovePanel(win) {
	...
	/* 指定方向へ即時実行。Option＋クリックで複製、通常は移動。マージン／プレビュー境界はオプションパネルの設定を使用 / Run immediately; Option-click duplicates, otherwise moves; margin / preview-bounds come from the Options panel */
	...
	/* 中央ボタン（複製）を onDraw で描画（ホバーで背景変化）/ Draw the center (duplicate) button via onDraw (background changes on hover) */
	function drawCenterButton(control) {
		var graphics = control.graphics;
		var w = control.size[0], h = control.size[1];
		var backgroundColor = (control.isHover === true) ? uiColors.hoverBackground : uiColors.background;
		graphics.rectPath(0, 0, w, h);
		graphics.fillPath(graphics.newBrush(graphics.BrushType.SOLID_COLOR, backgroundColor));
		if (uiColors.border) {
			graphics.rectPath(0.5, 0.5, w - 1, h - 1);
		...
		drawDuplicateGlyph(graphics, w, h, uiColors.foreground, backgroundColor);
	}

	/* 選択中のオブジェクトを同じ座標に複製し、複製側を選択状態にする（方向 none＝オフセット0）。DOM 処理は既存 worker に委譲 */
	/* Duplicate the selection in place (same coordinates) and select the copies (direction none = zero offset); delegate the DOM work to the existing worker */
	function executeDuplicateInPlace() {
		if (isBusy) { return; }
		isBusy = true;
		try {
	...
	/* 中央（複製）ボタンを追加 / Add the center (duplicate) button */
	function addCenterButton(row) {
		var button = row.add('iconbutton', undefined, undefined, { style: 'toolbutton' });
		button.preferredSize = [CELL_SIZE, CELL_SIZE];
		button.isHover = false;
		button.helpTip = L('tooltip.duplicateInPlace');
		button.onDraw = function () { drawCenterButton(this); };
		button.onClick = function () { executeDuplicateInPlace(); };
		attachHover(button);
	}
	...
	function addDirectionButton(row, directionKey) {
		...
		button.isHover = false;
		button.helpTip = L('direction.' + directionKey) + '  —  ' + L('tooltip.moveDuplicate');
