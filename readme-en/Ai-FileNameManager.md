...
[![Japanese](https://img.shields.io/badge/README-Japanese-4b8bbe.svg)](https://github.com/swwwitch/illustrator-scripts/blob/master/readme-ja/Ai-FileNameManager.md)
...
Only Illustrator format (.ai) is ever written, so **"Rename Original" and "Save a Copy" are unavailable when a non-.ai document is open (PDF, EPS, SVG, .ait, and so on).** The former would convert the format and then delete the original; the latter would produce a file whose contents are still in the original format but whose extension says .ai. Use "Save As" in that case — the original is kept.
...
A v-number or sequence is recognized only when it is **delimited by a separator (`-` `_` `.`) or by the ends of the name**. In `rev1-catalog-v02.ai`, the `rev1` is "revision 1" rather than a version, so the trailing `v02` is what gets bumped.
...
The active Illustrator document. The output is always .ai. When a non-.ai document is open, only "Save As" is available.
