...
A persistent palette that aligns the selection to the artboard. It offers align icons and eight move buttons, and each press steps the destination outwards: the margin, the artboard edge, then the bleed. When the Align panel is not set to Align to Artboard, that is detected and reported, and nothing is moved.
...
- Move: eight arrow buttons in a 3×3 grid (the four sides and the four corners) move the selection to the guide it would run into in that direction, or to the artboard edge. A diagonal moves to both edges at once. Unlike aligning, nothing is grouped, so the stacking order and the layers are untouched
...
- The arrow buttons move the selection so that its edge on that side lands on a guide; with no guide it goes to the artboard edge. The margin is not used. With Add Guides on, the first press stops at the margin guide and a second one carries on to the artboard edge beyond it.
...
- A clipping group is measured by **the bounds of its mask path**. The bounds Illustrator's DOM reports for a clipping group cover the whole content and ignore the mask, which would otherwise send the arrow buttons to the wrong place (the seven align icons already line up with the mask, because the align command does). A compound path mask is handled as well; with no mask found the ordinary bounds are used.
