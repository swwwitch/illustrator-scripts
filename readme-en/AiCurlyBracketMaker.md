...
Creates a curly bracket path. The on-artboard preview updates as the radii, length and stroke settings change, and Create commits exactly what is shown. Running it with a selection seeds the size and direction from that selection and hugs its edge.
...
- Center Offset slides the middle point along the length while the overall length holds
...
- Automatic placement against the selection, with an adjustable margin
- Brackets are marked, so selecting one and running again redraws it in place, facing the same way
- Arrow keys step the numeric fields (Shift for 10s, Option for 0.1)
...
| --- | --- |
| A bracket this script made | A redraw. Rebuilt on the same center and facing the same way; the original is removed on Create (Margin is dimmed) |
...
| Nothing | The bracket is centered on the artboard (Margin is dimmed) |
...
The bracket is placed so that its arm ends touch the edge of the selection. Margin sets the gap from that edge.
...
| Item | Description |
| --- | --- |
| Radius (Center) | Radius of the arcs that form the point in the middle (mm) |
| Radius (Ends) | Radius of the arcs that curl outward at both ends (mm). **0 leaves a right angle** with no arc. While Link is on it follows the center radius and cannot be edited |
| Direction | Where the middle point faces (up / down / left / right) |
| Center Offset | Moves the middle point along the length (mm, 0 keeps it centered). Up for a left/right bracket, right for an up/down one; negative reverses. The overall length is unchanged |
| Length | Overall length of the bracket (mm). The straight sections are derived as "length / 2 − end radius − center radius" |
| Extension | Straight run added at both ends, away from the middle (pt). With a zero end radius it turns at a right angle |
| Margin | Gap between the selection and the bracket (mm) |
