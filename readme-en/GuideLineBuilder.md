...
- With "On a separate layer" on, the output goes to a `_construction_guide` layer. If the selection is already on that layer, the existing one is renamed to `_construction_guide_backup...` and a fresh layer is created; the original objects are kept.
