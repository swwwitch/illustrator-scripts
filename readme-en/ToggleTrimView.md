# Toggle Trim View and show the guides

[![Direct](https://img.shields.io/badge/Direct%20Link-ToggleTrimView.jsx-ffcc00.svg)](https://github.com/swwwitch/illustrator-scripts/blob/master/jsx/misc/ToggleTrimView.jsx)

[![Japanese](https://img.shields.io/badge/README-Japanese-4b8bbe.svg)](https://github.com/swwwitch/illustrator-scripts/blob/master/readme-ja/ToggleTrimView.md)

[![Direct](https://img.shields.io/badge/Back%20to%20home-All%20scripts-cccccc.svg)](https://github.com/swwwitch/illustrator-scripts/blob/master/README.md)

---

### Overview

Toggles Trim View and shows the guides.

### Usage

1. Open the document.
2. Run the script.

### Notes

- A short script that simply runs the `TrimView` and `showguide` menu commands.

### Update History

- v1.0
