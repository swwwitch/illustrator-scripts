...
[![English](https://img.shields.io/badge/README-English-4b8bbe.svg)](https://github.com/swwwitch/illustrator-scripts/blob/master/readme-en/AiAdjustVerticalGapPalette.md)
...
- 「キーオブジェクト」（上／下／自動判定）を基準に、もう一方を移動します
- 「自動判定」（既定）は Illustrator で設定したキーオブジェクトを判定して基準にします。2つ選択時のみ有効で、整列コマンドで一時的に動かして判定し、元の位置へ戻します。判定できないときは「上」になります
...
- キー操作：T＝上／B＝下／K＝自動判定、N／L／C／R＝整列しない／左／中央／右、S＝整列に連動／J＝均等配置、A＝適用、Esc＝閉じる（ロック中はパネル系ショートカットは無効）
...
app.undo() してから再適用する。［適用］も同じ送信の中で取り消してから適用する。
確定時（［適用］）は行揃えを取得値で判定せず必ず代入する。undo 直後の
paragraphAttributes.justification は取り消し前の値を返すことがあり、
「すでに左揃え」と誤判定して代入を飛ばすと左揃えが戻ってしまうため。
...
- v1.3.3（2026-09-26）ファイル名を `AiAdjustVerticalGap.jsx` から `AiAdjustVerticalGapPalette.jsx` に変更。
- v1.3.2（2026-09-25）パレットの参照を常駐エンジンに保持し、CloseAllPalettes.jsx から閉じられるようにした。
