...
[![English](https://img.shields.io/badge/README-English-4b8bbe.svg)](https://github.com/swwwitch/illustrator-scripts/blob/master/readme-en/AreaTypeToolkit.md)
...
手作業だと、長方形を描いて → エリア内文字にして → 元のテキストを流し込んで → フォントを設定し直して……と手数がかかります。そのうえ、エリア内文字にしたあとの調整（フレームサイズ、行送り、インデント、垂直方向の配置）は、パネルを行き来しないと終わりません。

このスクリプトは、**作成と調整をひとつの流れにまとめた**ツールキットです。

<img alt="エリア内文字を調整ダイアログの外観" src="../png/ss-868-1198-144-20260728-182808.png" width="50%" />
...
パス上文字は、いったんポイント文字へ分離してから処理します。文字ごとのフォント・サイズ・塗り・線・行送りを退避して復元するので、見た目は保たれます。分離するのは**［変換］を押した時点**なので、［キャンセル］で閉じたときは元のパス上文字がそのまま残ります。
...
- 左：フォントサイズ／フレームサイズ／オフセット／垂直方向の配置
- 右：種別／行送り／行揃え／インデント／日本語の組版
- 下：［テキストを分離...］／［キャンセル］［OK］

［OK］で確定、［キャンセル］で元に戻して閉じます。ただし、垂直方向の配置・自動サイズ調整・種別・文字あふれ解消・枠にフィット・テキストを分離は、押した時点でその場に反映されます（後述）。

エリア内文字を選択して開いたときは、**触っていない項目には手を加えません**。垂直方向の配置・高さ・禁則・文字組みは、値を変えるか種別を選ぶまで現状のままです（垂直方向の配置と自動サイズ調整はDOMから読み取れないため、初期表示が実際の設定と一致しないことがあります）。

### 種別

用途ごとの設定を1クリックでまとめて適用します。

| 種別 | 行送り | 行揃え | 禁則 | 文字組み | 垂直方向の配置 | タブ |
| --- | --- | --- | --- | --- | --- | --- |
...
行揃え（左揃え・中央揃え・右揃え・均等配置（最終行左揃え）・両端揃え）と、垂直方向の配置（上揃え・中央揃え・下揃え・均等配置）を設定します。

行揃えは Illustrator の段落パネルと同じ**アイコンボタン**です。名称はツールチップで確認できます。

行揃えは **L / C / R / J / F** キーでも切り替えられます（入力欄にフォーカスがあるときは無効）。

垂直方向の配置は DOM から安定して設定できないため、ダイナミックアクション（`adobe_frameAlignment`）を一時的に読み込んで適用しています。実行時に一時ファイルとして書き出し、終了時に破棄するので、アクションパネルには残りません。アクションセット名は `AreaTypeToolkit_Alignment` / `AreaTypeToolkit_AutoSize` で、既存のアクションと衝突しません。

DOM から現在の配置を読み取ることはできないため、エリア内文字を選択して開いたときは常に「上揃え」が選ばれた状態になります。**アイコンをクリックするまでフレームの配置は変わりません。**
...
エリア内文字を選択して開いたときは、そのフレームの現在の設定を表示します。未設定なら「なし」を表示し、触らないかぎり変更しません。変換直後のフレームには、スクリプト冒頭の `DEFAULT_KINSOKU`（弱い禁則 v2）と `DEFAULT_MOJIKUMI_INDEX`（ベタ組み）が初期値として適用されます。
...
高さの下にある［自動サイズ調整］は、エリア内文字の高さをテキスト量に追従させる設定です。ダイナミックアクション（`adobe_SLOAreaTextDialog`）で切り替えるため、プレビューからは呼べず、**チェックした時点でその場に反映**します（プレビューをいったん外し、適用してから貼り直す）。

ONのあいだ、**高さ欄はディム**します（枠が文字に追従するため、こちらから指定できません）。高さに触らないので、プレビューが枠を引き戻すこともありません。

自動サイズ調整の現在の状態も DOM からは読み取れないため、チェックボックスは常にOFFで開きます。エリア内文字を選択して開いたときは、**高さ欄を編集するまで高さを枠へ書き戻しません**（すでに自動サイズ調整がONのフレームと衝突しないようにするため）。

### インデント／オフセット

- インデント：左・右（「連動」は既定でON。右は左に追従し、右の欄はディムします。左右が違う値のテキストを選んで開いたときは、連動を外した状態で開きます）
- オフセット：フレームとテキストの距離（チェックを入れると入力できます）
...
- 文字組みの「なし」は設定名がUI言語で変わるため、`なし` → `None` の順に試します。

## 更新履歴

- v1.2.2（2026-08-03）変換ダイアログでキャンセルしたときにパス上文字が戻らない不具合を修正（分離は［変換］確定時に実行）。既存のエリア内文字を開いてOKしたときに、垂直方向の配置が上揃えに戻ったり、禁則・文字組みが初期値で上書きされたりする不具合を修正。ダイナミックアクションのセット名を `AreaTypeToolkit_AutoSize` / `AreaTypeToolkit_Alignment` に変更し、同名のユーザーアクションを削除しないよう修正。「選択した図形に流し込む」で複合パスを扱えるよう修正。文字組み「なし」の設定名を日英どちらでも扱えるよう修正
