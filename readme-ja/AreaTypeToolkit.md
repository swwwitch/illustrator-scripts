...
［OK］で確定、［キャンセル］で元に戻して閉じます。ただし、テキストの配置（垂直方向）・自動サイズ調整・種別・文字あふれ解消・枠にフィットは、押した時点でその場に反映されます（後述）。
...
| 選択肢 | 動作 |
| --- | --- |
| 分離しない | 分解せず、以下の調整だけを行う（既定） |
| 枠を1pt黒に | 分解し、長方形を1pt・黒の線にする |
| 枠を塗り・線なしに | 分解し、長方形を塗り・線なしにする |
| 枠を削除 | 分解し、長方形を削除する |

「分離しない」以外を選ぶと、他の調整パネルはディムします。
...
どちらも押した時点で確定し、結果のサイズをフォントサイズ欄に書き戻します。最小サイズでも収まらなかったときは、極小のまま残さず元のサイズに戻します。
...
禁則の「なし」は Illustrator のスクリプティングから設定できないため、選んでも反映されません（他の値からの切り替えは可能です）。
...
- v1.2.0（2026-07-28）「種別（本文／見出し／メニュー）」「行送り」「日本語の組版（禁則・文字組み）」パネルを追加。行揃えとテキストの配置をアイコンボタン化。リーダー罫のチェックボックスを「メニュー」種別に統合。自動サイズをチェックボックスにし、クリックで即反映するよう変更。「内側のマージン」を「オフセット」に改称。文字あふれ解消・枠にフィットを改行を含むテキストで中止し、結果をフォントサイズ欄に反映するよう修正。UI文言をIllustratorの用語に合わせ、各項目にツールチップを追加。既存エリア内文字の行揃え・インデントを正しく読み込むよう修正。「選択した図形に流し込む」を複数組に対応。Q（歯）単位に対応。変換できないときの警告を追加
