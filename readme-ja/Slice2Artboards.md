...
[![English](https://img.shields.io/badge/README-English-4b8bbe.svg)](https://github.com/swwwitch/illustrator-scripts/blob/master/readme-en/Slice2Artboards.md)
...
- オフセットとマージンの入力欄は定規単位のラベルを表示しますが、**値はポイントとして扱われます**。定規がmmのときに「10 mm」と表示されていても、実際に適用されるのは10ptです。
- 行数・列数に数値以外を入力した場合は、メッセージを表示して中断します。片方だけを0にすると、選択物の縦横比からもう一方を自動計算します。
- 「アートボードに変換」を有効にすると、分割前からあったアートボードは削除されます。
- 複数のオブジェクトを選択した場合は、いったんグループ化してからシンボルに変換して処理します。
...
- v1.5 (20260727): 効果のなかった `app.undoGroup` への代入を削除（Illustratorに該当プロパティは存在せず、取り消しはまとまりません）
- v1.6 (20260727): 未使用の旧オフセット処理（`createOffsetEffectXML` / `applyOffsetPathToSelection`）を削除。どこからも呼ばれておらず、呼ぶとスコープ外の関数を参照してエラーになる状態でした（オフセットは従来どおり矩形の座標調整で行われます）。あわせてコードを整理し、日本語のままだったUI文言（「ファイル名を参照」「記号」「なし」）を日英対応に変更、ドキュメント未オープン時・未選択時にメッセージを表示するように追加しました