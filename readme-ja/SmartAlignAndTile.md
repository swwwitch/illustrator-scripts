...
- ランダム配置（全体の左上位置は保持）
- 数値欄を↑↓キーで増減（Shift+↑↓で10単位にスナップ）。行数・列数は1以上に丸めます
...
- **プレビュー境界を使用**：ONで線幅や効果を含む境界（visibleBounds）、OFFで幾何境界（geometricBounds）を基準にします。Illustratorの環境設定を一時的に切り替えますが、OK・キャンセルのどちらでも元の設定に戻します。
...
- 行数・列数で割り切れない場合、余りは前の行（列）から1つずつ多く配分されます（指定した行数・列数は必ず使い切ります）。
...
- v2.0.0 (20260906) : `SmartAlignAndTile-yoko`（v1.7.1）と `SmartAlignAndTile-tate`（v1.8）を統合し、方向（横／縦）の切り替えに対応。キーオブジェクトを配置の起点にするよう変更、揃えに「なし」を追加、行・列の基準を最大サイズに統一。あわせて Shift+↓のスナップ、行数・列数の下限と丸め、指定した行数・列数の使い切り、環境設定の復元を修正
