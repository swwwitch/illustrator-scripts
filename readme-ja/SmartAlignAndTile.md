...
- **キーオブジェクトを基準**：キーオブジェクトを配置の起点にします。キーオブジェクトは動かず、ほかのオブジェクトが横なら右へ、縦なら下へ並びます。判定できないときはディムされます。
...
- v2.0.0 (20260905) : `SmartAlignAndTile-yoko`（v1.7.1）と `SmartAlignAndTile-tate`（v1.8）を統合し、方向（横／縦）の切り替えに対応。キーオブジェクトを配置の起点にするよう変更、揃えに「なし」を追加、行・列の基準を最大サイズに統一
