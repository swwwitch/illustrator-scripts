...
- すべてのプリセットを形状アイコンで表示
- 台形4種類
- 平行四辺形8種類（固定する角4か所 × 横／縦）
- 三角形4種類
- 対角線2種類
- 変形量を `0%`〜`49%` の範囲で調整（強度を掛けた実効値を併記）
- 強度を「弱」「標準」「強」から選択
- Undoベースのプレビュー
- 複数オブジェクトへの一括適用
- 日本語／英語UI
...
3. ダイアログでプリセットを選択します。
4. 台形または平行四辺形では、［変形の調整］で変形量と強度を設定します。
...
スライダーの下には `20%（実効 10%）` のように、スライダーの値と実効値を並べて表示します。実効値はスライダーの値に上記の倍率を掛けたもので、平行四辺形ではさらに2倍されます。
