...
### 概要
    ...
    - Illustrator のアートボード／シンボル／レイヤー／グラフィックスタイル名を、接頭辞・接尾辞・名前ソース・検索置換を組み合わせて一括リネーム／並び替えするスクリプトです。
...
### 並び替え／リネーム（右カラム下段）
    ...
    - チェック行を「↑先頭へ／↑上へ／↓下へ／↓末尾へ」で並び替え。アートボードは rect 入れ替え、レイヤーは `.move()` で並び替え（シンボル／グラフィックスタイルは並び替え不可）
...
### 更新履歴
    ...
    - v1.6.2 (20260923) : 行の「新しい名前」をフォーカスしたまま［更新］／OK しても手動編集として反映されるよう修正。検索フィルター ON で手動で付け外ししたチェックが［更新］で上書きされる問題、検索フィルター ON 中に「指定範囲」欄が有効に戻る問題、`{#01}` 連番が100件目から "00" に切り詰められる問題を修正。シンボル／グラフィックスタイルは並び替えできないため並び替えボタンを無効化（並び替えが効かないまま別の項目がリネームされていた）。一覧を12行表示＋スクロールバーに変更（項目が多いと下の行が表示されなかった）。「元の名称」で何も入力せずに OK したとき、同名に "_1" が付かないよう入力なしとして扱い、ダイアログを閉じる前に警告するよう変更。アートボードが重なっているとき「最前面のテキスト」が別のアートボードに割り当てられる問題、アートボードの位置の復元に失敗すると一時名が残る問題を修正
