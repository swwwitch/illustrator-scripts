...
- 切り口はワープ（旗・上昇・直線）で曲げたり、ジグザグでギザギザにしたりでき、切り口に沿った省略線も引けます。

<img alt="省略線でトリミングダイアログの外観" src="../png/ss-840-936-144-20260925-192454.png" width="50%" />

### 主な機能

- マスク用のパスの幅（左右に切るときは高さ）でトリミング。%で伸縮も可能
- 切る方向を自動判定（上下／左右）
- 切り口の形を「旗」「上昇」「直線」「ギザギザ」から選択、カーブの量を%で、ギザギザは大きさと折り返しで指定
...
| 項目 | 説明 |
| --- | --- |
| 幅／高さ（切り口に沿った方向） | オブジェクトを切りそろえる範囲の大きさ（%）。100%でマスク用の図形のとおり。上下に切るときは幅、左右に切るときは高さ |
| 高さ／幅（切る方向） | 取り除く範囲の大きさ（%）。100%でマスク用の図形のとおり。上下に切るときは高さ、左右に切るときは幅 |
| 上下位置／左右位置 | マスク用の図形の位置（定規の単位）。プラスで下・右へ、マイナスで上・左へ |
| 旗／上昇／直線／ギザギザ | 切り口の形。「旗」は波形、「上昇」は片側へせり上がるカーブ、「直線」は斜めの直線、「ギザギザ」はジグザグ |
| カーブ | 切り口を曲げる量（-100〜100%）。「直線」では傾きの量。0でまっすぐに切り、マイナスで逆向きになる |
| 大きさ | 「ギザギザ」の山の高さ（定規の単位）。0でまっすぐに切る |
| 折り返し | 「ギザギザ」の、切り口の端から端までの折り返しの数（0〜100）。0でまっすぐに切る |
| 間隔 | 詰めたあとの2つのパーツのあいだの距離（定規の単位）。片側だけ残すときは使わない |
| 省略線を追加 | 切り口に沿った省略線を引く |
| 線幅 | 省略線の線幅（「線」の単位） |
| 線端 | なし（線端なし）／丸型（丸型線端）。丸型にすると線の両端（破線は各線分の端）が丸くなる |
| 線種 | 実線／破線 |
...
- v1.0.8 (20260925) : 切り口の形に「ギザギザ」（ジグザグ効果）を追加し、大きさと折り返しを指定できるようにした。切り口の形のラジオボタンを2行2列に並べた
- v1.0.7 (20260923) : 「マスク用の図形」パネル（旧「マスクの図形」）に、切り口に沿った方向の大きさ（上下に切るときは幅、左右に切るときは高さ）を追加。スタイル「上昇（直線）」を「直線」に改名し、項目名「スタイル」を外して旗・上昇・直線を横並びにした。UIの文言を見直し、タイトルを「省略線でトリミング」、パネル名「トリミング」を「切り口」に変更。ツールチップを実際の動作に合わせて書き直し、パネル・スタイル・項目名にも追加
- v1.0.6 (20260921) : 左右に切り分けたときは、左側のパーツを固定して右側を寄せるように変更
- v1.0.5 (20260921) : カーブに負の値を許容（-100〜100%）。マイナスにすると逆向きに曲がる
