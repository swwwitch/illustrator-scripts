...
キーワード・ウェイト・種類のいずれでも絞り込まずに実行すると全フォントが対象になるため、実行前に確認ダイアログが表示されます。フォント数によっては非常に時間がかかります。

ファミリーごとの列幅と行の高さは内容に合わせて決まるため、ウェイトの多いファミリーや長いサンプル文字でも重なりません。
...
| `"…"` | 空白・ハイフンを無視して一致（語句をひとまとめに） | `"Helvetica Neue"` |
...
| --- | --- |
| 基本 | Text、Headline |
| 狭める系 | Cond、Condensed、Compressed、Comp（Compact は除く） |
...
- 絞り込み条件なしで実行すると、環境によっては数千書体が対象になり、処理に時間がかかります。
...
- 環境にないフォント（ドキュメントで使われているだけの、置き換え用の仮エントリ）は対象外です。
